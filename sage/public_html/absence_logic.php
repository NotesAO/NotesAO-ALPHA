...
function buildAbsenceRecords()
{
    ...
    $sql = "SELECT id, orientation_date, first_attendance.date first_attendance, weekly_attendance, attends_sunday, attends_sunday_t4c, attends_monday, attends_tuesday, attends_wednesday, attends_thursday, attends_friday, attends_saturday from client c 
        ...
        where c.exit_date is null
          ...
          and c.required_sessions > sessions_attended.count";
...
function buildAbsenceRecordsForClient($client_id, $start_date, $weekly_attendance, $row)
{
    ...
    while ($date_idx < $stop_date) {
        $need = sessionsRequiredThatDay($date_idx, $row);  // 0,1 or 2
        for ($i = 0; $i < $need; $i++) {
            $required_days[] = clone $date_idx;            // push once or twice
        }
...
/**
 * How many separate sessions should this client attend on $d ?
 * Sunday can be 0, 1 or 2; every other day is 0 or 1.
 */
function sessionsRequiredThatDay(DateTime $d, array $row): int
{
    switch ($d->format('w')) {              // 0 = Sun … 6 = Sat
        case '0':  // Sunday
            return ($row['attends_sunday']     ? 1 : 0)
                 + ($row['attends_sunday_t4c'] ? 1 : 0);
        case '1':  return $row['attends_monday'];
        case '2':  return $row['attends_tuesday'];
        case '3':  return $row['attends_wednesday'];
        case '4':  return $row['attends_thursday'];
        case '5':  return $row['attends_friday'];
        case '6':  return $row['attends_saturday'];
    }
    return 0;
}
