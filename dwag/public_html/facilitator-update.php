...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE facilitator SET first_name=?,last_name=?,email=?,phone=?,licensure=? WHERE id=?");
    ...
    if(!$stmt->execute([ $first_name,$last_name,$email,$phone,$licensure,$id  ])) {
        ...
        $stmt = null;
        header("location: facilitator-read.php?id=$id");
    }
} else {
    ...
    if(isset($_GET["id"]) && !empty($_GET["id"])){
        ...
        $sql = "SELECT * FROM facilitator WHERE id = ?";
...
<head>
    <meta charset="UTF-8">
    <title>NotesAO - Facilitator Update</title>
