<?php
/**
 * DWAG – BIPP Virtual Intake Packet
 * ------------------------------------
 * • GET  : render multi‑step form  (HTML follows this PHP section)
 * • POST : CSRF check → validate → INSERT into clinicnotepro_dwag.intake_packet
...
/* ------------------------------------------------------------------ */
/* Lookup: programs and therapy groups                                 */
/* ------------------------------------------------------------------ */
$programs = [];
$prog_rs = $db->query("SELECT id, name FROM program ORDER BY id");
while ($r = $prog_rs->fetch_assoc()) { $programs[(int)$r['id']] = $r['name']; }

$groups = [];            // flat list
$groups_by_program = []; // program_id => [rows]
$grp_rs = $db->query("SELECT id, program_id, name FROM therapy_group ORDER BY name");
while ($g = $grp_rs->fetch_assoc()) {
    $g['id'] = (int)$g['id']; $g['program_id'] = (int)$g['program_id'];
    $groups[] = $g;
    $groups_by_program[$g['program_id']][] = $g;
}

/* For client-side filtering */
$GROUPS_JSON = json_encode($groups, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);

/* ------------------------------------------------------------------ */
/* Payment schedule options – EXACT DWAG language + fees               */
/* ------------------------------------------------------------------ */
$DWAG_PAYMENT_OPTIONS = [
  'A' => ['label'=>'$35,001 and up','fee_cents'=>3500],
  'B' => ['label'=>'$20,001 to $30,000','fee_cents'=>2500],
  'C' => ['label'=>'$15,001 to $20,000','fee_cents'=>1500],
  'D' => ['label'=>'$5,001 to $15,000','fee_cents'=>1000],
  'E' => ['label'=>'$1 to $5,000','fee_cents'=>null],
  'F' => ['label'=>'Indigent/Unemployed','fee_cents'=>null],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach (['first_name','last_name','date_of_birth','email','digital_signature'] as $req) {
        if (postv($req) === null) exit("<h3>Missing required field: $req</h3>");
    }
    if (in_array('Other', $_POST['reasons'] ?? [], true) && postv('other_reason_text') === null) {
        exit('<h3>Please explain the “Other” reason.</h3>');
    }



    $gender = postv('gender_id'); // still captured for record

    $program_id = (int)($_POST['program_id'] ?? 0);
    if (!$program_id) {
        // fallback to gender if user somehow bypasses the control
        $program_id = ($gender === '3') ? 3 : 2;
    }

    /* group selection comes as group_id; store its name in chosen_group_time */
    $group_id = (int)($_POST['group_id'] ?? 0);
    $chosen_group_time = null;
    if ($group_id) {
        // resolve from the prefetched $groups list
        foreach ($groups as $g) { if ($g['id'] === $group_id) { $chosen_group_time = $g['name']; break; } }
    }
    if (!$chosen_group_time) { exit('<h3>Please select your preferred group time.</h3>'); }
    // ---- Payment selection → DB fields
    // ---- Payment selection → DB fields
    $income_key = postv('income_bracket_key');           // 'A'..'F'
    $opt = $income_key ? ($DWAG_PAYMENT_OPTIONS[$income_key] ?? null) : null;
    if (!$opt) { exit('<h3>Please select your income bracket.</h3>'); }
    ...
    $fields = [
        ...
        'chosen_group_time'     => $chosen_group_time,
        'income_bracket'        => $opt['label'] ?? null,
        'session_fee_cents'     => isset($opt['fee_cents']) && is_int($opt['fee_cents']) ? (string)$opt['fee_cents'] : null,
        'consent_telehealth'    => postb('agree_telehealth'),
        'consent_acknowledgement'  => postb('agree_acknowledgement'),
    ...
    $headers  = "From: reporting@dwag.notesao.com\r\n";
    ...
    @mail(
        ADMIN_ALERT_EMAIL,
        "DWAG Group has received a new Intake Packet for $fname $lname",
        "A new online intake packet was submitted.\n\nView pending & submitted packets:\nhttps://{$_SERVER['HTTP_HOST']}/intake-index.php",
        $headers,
        '-freporting@dwag.notesao.com'            // ← some MTAs (esp. cPanel) require this
    );
...
if (!empty($_SESSION['show_thank_you_once'])) {
    ...
    <div class="jumbotron bg-white text-center shadow-sm py-4 mb-4">
      <a href="https://dwagbipp.org/" target="_blank" rel="noopener">
        <img src="dwaglogo.png" alt="DWAG"
    ...
    <div class="card shadow-sm">
      <div class="card-body text-center p-5">
        ...
        <a href="https://dwagbipp.org/" class="btn btn-primary btn-lg">
          DWAG Home Page
        </a>
      </div>
    </div>
...
<style>
 ...
 .invalid-field{border:2px solid #d93025 !important;background:#ffecec !important;}
  ...
  .seg-radio{display:grid;grid-template-columns:repeat(4,minmax(11rem,1fr));gap:.5rem;justify-content:center}
  .seg-item{position:relative} /* wrapper for each input+label */

  .seg-radio input[type=radio]{position:absolute;opacity:0;pointer-events:none}
  .seg-radio label{
    border:1px solid #cbd3da;border-radius:.5rem;padding:.6rem .9rem;
    cursor:pointer;background:#fff;text-align:center;
    box-shadow:0 1px 2px rgba(0,0,0,.04);font-weight:600
  }
  .seg-radio label .fee{display:block;font-weight:500;font-size:.85rem;color:#555}
  .seg-radio input[type=radio]:focus + label{outline:2px solid #80bdff;outline-offset:2px}
  .seg-radio input[type=radio]:checked + label{background:#007bff;color:#fff;border-color:#007bff}
  .seg-radio input[type=radio]:checked + label .fee{color:#eaf2ff}

  /* Center the last row (works when there are exactly 6 options: 4 + 2) */
  .seg-radio .seg-item:nth-last-child(2){grid-column:2}
  .seg-radio .seg-item:last-child{grid-column:3}
...
<!-- Jumbotron / Header Section -->
    <div class="jumbotron bg-white text-center shadow-sm py-4">
      <a href="https://dwagbipp.org/" target="_blank">
        <!-- Responsive Image -->
          <img 
              src="dwaglogo.png" 
              alt="DWAG" 
...
<h1>DWAG Program Intake Packet</h1>
<form method="post">
  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(),ENT_QUOTES) ?>">

    <!-- ===== Introduction / Instructions ===== -->
    <div class="intro">
    <p><strong>This form is an Intake and Orientation document for the Dutch-Will Assessment Group’s Battering Intervention and Prevention Program (BIPP) & MRT.</strong> It outlines the program’s requirements, participant obligations, confidentiality agreements, and other essential information. By completing this form, participants acknowledge their understanding of the program’s rules and commitments necessary for successful participation and completion.</p>
    ...
    <p>If you have questions while filling out the form or encounter any technical difficulties, please contact our support team at: <strong>(409) 658-8384</strong>.</p>
  ...
  <fieldset class="step">
    ...
    <div class="row">
      <div class="col">
        ...
        <small class="form-text text-muted">BIPP (male), BIPP (female), or MRT.</small>
      </div>

      <div class="col">
        <label class="required">Preferred Group Time</label>
        <select name="group_id" id="group_id" class="form-select" required>
          <option value="" disabled selected>Select a program first…</option>
          <?php foreach ($groups as $g): ?>
            <option value="<?= (int)$g['id'] ?>" data-program="<?= (int)$g['program_id'] ?>">
              <?= htmlspecialchars($g['name']) ?>
            </option>
          <?php endforeach; ?>
        </select>
        <small class="form-text text-muted">List filters based on Program.</small>
      </div>
    </div>

    <script>
      /* Populate & filter group times by selected program */
      (function(){
        const GROUPS = <?= $GROUPS_JSON ?>; // [{id,program_id,name}, …]
        const programSel = document.getElementById('program_id');
        const groupSel   = document.getElementById('group_id');

        function resetGroups() {
          for (const opt of Array.from(groupSel.options)) {
            if (!opt.value) continue; // keep placeholder
            const show = (String(opt.getAttribute('data-program')) === programSel.value);
            opt.hidden = !show;
          }
          groupSel.value = '';
        }

        programSel.addEventListener('change', resetGroups);
        // On first load, restore saved selections from localStorage if present
        document.addEventListener('DOMContentLoaded', () => {
          // if program was restored by your existing LS logic, call filter once
          if (programSel.value) resetGroups();
        });
      })();
    </script>
    ...
    <h3 style="margin-top:2rem">Payment Schedule</h3>
    <div class="policy">
      <p>Dutch-Will Assessment Group-BIPP requires participants to provide payment prior to receiving services.</p>
      <p>Minimal payment will be required for you as it serves as a financial means of accountability for your abusive behaviors.</p>
      <p>Dutch-Will Assessment Group-BIPP will operate on sliding scale pricing.</p>
    </div>

    <label class="required">Income</label>
    <div class="seg-radio" role="radiogroup" aria-label="Income">
      <?php $i=0; foreach ($DWAG_PAYMENT_OPTIONS as $key => $o): ?>
        <div class="seg-item">
          <input type="radio" name="income_bracket_key"
                id="income_<?= htmlspecialchars($key) ?>"
                value="<?= htmlspecialchars($key) ?>" <?= $i++===0?'required':'' ?>>
          <label for="income_<?= htmlspecialchars($key) ?>">
            <?= htmlspecialchars($o['label']) ?>
            <span class="fee"><?= is_int($o['fee_cents']) ? ('$'.number_format($o['fee_cents']/100,0).' / session') : 'Fee set with facilitator' ?></span>
          </label>
        </div>
      <?php endforeach; ?>
    ...
    <div class="table-responsive mt-2">
      <table class="table table-sm table-bordered">
        <thead class="thead-light">
          <tr><th>Income</th><th>Session Fee</th></tr>
        </thead>
        <tbody>
          <tr><td>$35,001 and up</td><td>$35</td></tr>
          <tr><td>$20,001–$30,000</td><td>$25</td></tr>
          <tr><td>$15,001–$20,000</td><td>$15</td></tr>
          <tr><td>$5,001–$15,000</td><td>$10</td></tr>
          <tr>
            <td>$1–$5,000</td>
            <td>Dutch-Will Assessment Group-BIPP will work with participants to develop a session fee that is sensitive to participants who are significantly struggling financially.</td>
          </tr>
          <tr>
            <td>Indigent/Unemployed</td>
            <td>Dutch-Will Assessment Group-BIPP will work with participants to develop a session fee that is sensitive to participants who are indigent and unemployed.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="policy">
      <p>Dutch-Will Assessment Group-BIPP will not offer you additional fee-based services provided by either the organization operating or overseeing the accredited program or any of the program’s staff members.</p>
      <p>Referrals for additional fee-based services or supplemental programming will be made by referral sources.</p>
      <p>If I should experience any financial challenges throughout the program, I will notify my facilitator.</p>
    </div>
    ...
    outside Dutch Will Assessment Group – BIPP. This right applies with the following exceptions:</p>
    ...
    <ul>
        ...
        <li><strong>Media involvement:</strong> Any media contact arranged by the Dutch Will Assessment Group program
            will include the presence of a Dutch Will Assessment Group employee to protect victim confidentiality.</li>
    </ul>

    <p><strong>We ask that you keep confidential information you may learn about other clients who are
    receiving services from Dutch Will Assessment Group – BIPP.</strong></p>

    <p><strong>Dutch Will Assessment Group requires facilitators and participants to:</strong></p>
    <ul>
    ...
    <ul>
        ...
        <li>If unresolved, report concerns to your counselor's immediate supervisor, Executive Director
            Brenda Riley, at 409-658-8384.</li>
        <li>If further resolution is needed, contact the Texas Council on Family Violence at 800-525-1978.</li>
    </ul>

    <h6 style="margin:1rem 0 .5rem;font-weight:700">Additional DWAG Confidentiality Acknowledgements</h6>
    <p>I understand that all information about Dutch-Will Assessment Group LLC participants is confidential. I will not identify or discuss any identifying information pertaining to either a participant or victim (i.e., name, address, phone number, occupation, etc.) with anyone outside of Dutch-Will Assessment Group LLC staff volunteers and interns.</p>
    <p>I understand that all of the above-mentioned conditions apply to me while I am participating, employed, volunteering and/or interning with Dutch-Will Assessment Group LLC and continue to be binding after my current working relationship with Dutch-Will Assessment Group LLC ends.</p>
    <p>I have read and understand the above-mentioned information. Failure to honor this statement may result in possible prosecution.</p>
    ...
    I accept these terms as a condition of participation in the Dutch Will Assessment Group – Batterers Intervention &amp;
  ...
  <fieldset class="step" id="step-client-consents">
    ...
    <div class="mb-4">
      ...
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        ...
        <p>I understand that I may revoke this consent at any time and that my request for revocation must be in writing. If not earlier revoked, this consent for disclosure of information shall expire 1 year after my completion of or termination from Dutch Will Assessment Group - Batterers Intervention & Prevention Program.</p>
        <p>I understand my right to confidentiality. I further understand that this consent form gives Dutch Will Assessment Group - BIPP permission to share confidential information about me in the way described above.</p>
        <p>I understand that Victim will be contacted by the Victim Advocate and offered counseling services. She/He will be provided enrollment, completion, or termination information from Dutch Will Assessment Group - BIPP.</p>
        <p>Release of information is voluntary; I understand I have a right to refuse Dutch Will Assessment Group - BIPP's request for this disclosure.</p>
        <p>Dutch Will Assessment Group - BIPP reserves the right to dismiss any client who refuses to meet the provisions of The Texas Department of Criminal Justice-Community Justice Assistance Division and Texas Council on Family Violence Battering Intervention & Prevention Project guidelines.</p>
      ...
      <div class="form-check">
        ...
        <label class="form-check-label" for="agree_disclosure">
          By checking "I Agree", I confirm that I have read, understood, and agree to abide by the terms and conditions outlined above. I acknowledge my rights and responsibilities as described, and I accept these terms as a condition of participation in the Dutch Will Assessment Group - Batterers Intervention & Prevention Program.
        </label>
      </div>
    </div>

    <!-- Section: Dutch Will Assessment Group Program Agreement -->
    <div class="mb-4">
      <label class="form-label fw-bold">Dutch Will Assessment Group Program Agreement</label>
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        <p><strong>Program structure.</strong> Dutch-Will Assessment Group–BIPP consists of a minimum of 24 weekly 1.5-hour group sessions. Completion is required unless you are attending voluntarily. You must adhere to session rules and treat facilitators, participants, and guests with respect.</p>

        <p><strong>Fees.</strong> Assessment + Orientation fee: <strong>$50</strong>. Materials fee: <strong>$15</strong>. Per-session fee uses the sliding scale you selected in the Payment section. Fees are due at the start of each session. DWAG-BIPP may refuse admittance without payment, which can be recorded as an absence. Fees are not refunded once service is delivered. A <strong>$5.00 late charge</strong> is added for each class not paid at the time of service.</p>

        <p><strong>Attendance.</strong> You must complete all 24 sessions and arrive on time. Arrivals >10 minutes late are not admitted and count as an absence. More than three (3) absences results in dismissal and referral-source notification. After dismissal you restart from Session 1; if ≥6 months have passed since the initial assessment, a new assessment is required. Notify DWAG if you cannot attend.</p>

        <p><strong>Participation.</strong> You are responsible for engagement in the program. You will acknowledge your abuse, discuss relevant incidents, and participate in discussions and exercises regarding attitudes, beliefs, and behaviors.</p>

        <p><strong>Group rules.</strong> Participate in a non-abusive, non-controlling, non-intimidating manner; silence phones; refer to the victim by first name only; do not attend under the influence of alcohol or drugs; be present, on camera, in a private space, and follow facilitator instructions.</p>

        <p><strong>Communication.</strong> You must keep your contact information current and inform your facilitator of any violent incident or police call to your home due to a family disturbance.</p>

        <p><strong>Your rights.</strong> You have the right to dignity, respect, non-discrimination, clear explanations of goals, reporting, length, and costs, and a copy of your file upon written request. DWAG will strive to provide services in your primary language or refer you back to your referral source if unavailable.</p>

        <p><strong>Grievances.</strong> If you believe you were treated unfairly, notify your facilitator or the Executive Director (Brenda Riley, 409-658-8384). You may also contact the Texas Council on Family Violence at 800-525-1978.</p>

        <p><strong>Our obligations.</strong> DWAG will provide fair and humane treatment; copies of agreements and exit criteria; notice of changes to schedules; compliance with anti-discrimination laws; fee structure and schedule; grievance procedure; and progress reports to referral sources: an initial report within five (5) working days after assessment, monthly progress reports thereafter, and reports within five (5) working days for known law violations, incidents of physical violence, or termination.</p>
        ...
        <p>You must be focused and facing your webcam during the entire group; <strong>NO</strong> watching TV or working on your computer. You will only be given one warning. Second warning will result in no credit for the session and/or a discharge violation from Dutch Will Assessment Group BIPP program.</p>
        <p>You must be in a quiet room; not driving, in your car, or doing any other activity. Otherwise, you will receive no credit for that BIPP session and/or receive a discharge violation from Dutch Will Assessment Group BIPP Program.</p>
        <p>If there is any interruption during your Dutch Will Assessment Group by a child or adult, you will lose credit for your class and/or receive a discharge violation from Dutch Will Assessment Group Program.</p>
        <p>If there is any appearance of alcohol or vaping during Dutch Will Assessment Group, it will result in a request for a drug test. If found dirty, you must sign a behavioral contract or be unsuccessfully discharged from Dutch Will Assessment Group BIPP Program.</p>
        <p>During your Dutch Will Assessment Group, if there is any woman present even for a moment, especially the victim, you shall receive no credit for the class and/or be unsuccessfully discharged from Dutch Will Assessment Group BIPP Program.</p>
        <p>If after a restroom break you do not return, or you take more than 5 minutes, you shall receive no credit for your Dutch Will Assessment Group BIPP Group session.</p>
        ...
        <p>I will notify Dutch Will Assessment Group BIPP of any change of address or phone number.</p>
        <p>I hereby agree to contact BIPP by phone at 409-658-8384 when I am unable to attend a scheduled session. Failure to contact Dutch Will Assessment Group within 2 consecutive absences is an automatic discharge from Dutch Will Assessment Group Program.</p>
        ...
        <p>I understand that Dutch Will Assessment Group BIPP is committed to helping me gain a better understanding of my problems and how to find productive solutions and that it is the main goal of my psychoeducational classes.</p>
        <p>Disclosure may include notifications to victims’ advocates, referral sources, and law-enforcement or health professionals when required by law or safety.</p>
      </div>
      <div class="form-check">
    ...
    <div class="mb-4">
      ...
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        <p><strong>Accountability.</strong> You will acknowledge abusive behavior and discuss related incidents. You will not bring weapons or harmful objects. You will not use alcohol or illegal drugs while participating. You will not use violence, abusive, threatening, or controlling behaviors, including stalking or violating protective orders. You will adhere to group rules and stay current on program fees.</p>

        <p><strong>Virtual-session standards.</strong> Be on camera, focused, and in a private space. Do not record, take screenshots, or allow others to view or overhear group content. Disable smart listeners (e.g., Alexa, Siri, Google Assistant). Do not change locations during group. If others interrupt, relocate or end the interruption; credit may be denied.</p>

        <p><strong>Non-violence plan.</strong> You will develop and follow a non-violence plan per curriculum and practice skills covered in group.</p>
`
        ...
        <p>Dutch Will Assessment Group requires me to disable any devices that could collect information from the environment, such as Google Home Assistant, Amazon Alexa, or Apple Siri, and I agree that I will not record nor take screenshots of the group. Dutch Will Assessment Group requires me to be in a private space and not in any public area such as a park, yard, or open area; other people not in the group should not be exposed to the content nor hear or observe the group.</p>
    ...
    <div class="mb-4">
      ...
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        ...
        <p><strong>Completion requirements.</strong> Complete orientation and assessment; complete 24 sessions; make full payment of fees; comply with group rules; actively participate.</p>

        <p><strong>Grounds for termination.</strong></p>
        <ul>
          <li>Continued abuse, threats, or physical violence in any situation.</li>
          <li>Patterned manipulation or disruptive behavior during sessions.</li>
          <li>More than three (3) absences or repeated tardiness &gt;10 minutes.</li>
          <li>Violation of program or group rules.</li>
          <li>Non-compliance with program fees, including unpaid balances and late-payment issues.</li>
        </ul>

        <p><strong>Reporting.</strong> DWAG will notify referral sources of enrollment, completion, or termination. DWAG reports known law violations, incidents of physical violence, and terminations within five (5) working days. Monthly progress reports are provided during participation.</p>

        <p><strong>Confidentiality limits (summary).</strong> Information is confidential except as required by law or safety: subpoenas; imminent risk of harm to self/others; child, elder, or disabled-adult abuse/neglect; sexual exploitation by a provider; TDHHS/TDPRS and CJAD program assessments and research; and disclosures to victims, law enforcement, courts, medical/mental-health professionals, and referral sources when necessary for safety or as mandated.</p>

        <p><strong>Victim services notice.</strong> Victims identified on your assessment are contacted by the Victim Advocate and offered services. Enrollment, completion, or termination information may be shared with the victim advocate. No assessment details or in-group disclosures are shared with victims.</p>

        <p><strong>Acknowledgement.</strong> By agreeing, you confirm you have read and understand these policies and voluntarily enter services from DWAG-BIPP staff. By submitting the packet you attest the information provided is true under Texas law.</p>
        <p>I have received a copy of the "Policy for Clients" for Dutch Will Assessment Group - BIPP. I understand my rights and responsibilities, and I agree to enter Dutch Will Assessment Group - BIPP.</p>
        <p>I understand that in accordance with Guideline 31 of the Texas Department of Criminal Justice-Community Justice Assistance Division and Texas Council on Family Violence Battering Intervention Prevention Program guidelines, I am being provided a written agreement that clearly delineates the obligation of the Dutch Will Assessment Group - BIPP to the client. I understand that the Dutch Will Assessment Group - BIPP shall:</p>
        <ul>
        ...
        <p>As a client of Dutch Will Assessment Group - BIPP, you have the right to terminate services with our agency at any moment. The risk of terminating services will be explained to you by a counselor/instructor. You have the right to choose other agencies for your services, and Dutch Will Assessment Group - BIPP will provide you with a list of known community agencies that may provide the services you need, except for clients referred by Probation; clients will be referred back to their Supervision Officer.</p>
        <p>Dutch Will Assessment Group - BIPP also has the right to terminate services with clients if:</p>
        <ul>
          ...
          <li>Dutch Will Assessment Group - BIPP client violates any of the BIPP rules.</li>
        </ul>
        ...
        <p>Clients have the right to seek other resources outside of Dutch Will Assessment Group - BIPP, and when possible, Dutch Will Assessment Group - BIPP staff will provide or make a referral.</p>
        ...
        <p>Dutch Will Assessment Group - BIPP will provide batterers at the time of assessment (intake) with a copy of the circumstances under which they can be terminated before completion.</p>
        <p>I have read and understand the above statements and voluntarily enter into counseling services from the staff of Dutch Will Assessment Group - BIPP - by entering my name below. (By clicking SUBMIT, I hereby confirm the above information to the best of my knowledge is correct and true, with no misleading or false content in accordance with Texas Perjury Statute, Sec. 37.02 (a) (2) Chapter 32, Civil Practice and Remedies Code.)</p>
      </div>
      <div class="form-check">
  ...
  <fieldset class="step">
    ...
    <!-- 9A-2: Telehealth / Virtual Policy -->
    <h5 class="mt-4">Telehealth / Virtual Policy</h5>
    <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 300px; background-color: #f8f9fa;">
      <p>We are pleased to announce Dutch-Will Assessment Group LLC is offering telehealth/virtual services to our clients who wish to take advantage of this service.</p>
      <p>Virtual services will allow the client to attend weekly group sessions in the privacy of their home, enabling the client to keep all scheduled meetings and stay in compliance with DWAG LLC program rules and meeting the conditions of probation, parole, CPS, or your attorney’s recommendation.</p>

      <h6>Telehealth Privacy for Patients</h6>
      <p>Telehealth is a safe and secure way of connecting with your health care provider online. Just like in-person care, your telehealth appointments, messages, and information are protected by privacy rules.</p>

      <h6>Privacy During Your Appointment and Rules</h6>
      <ul>
        <li>You must be in a private, safe location where you feel comfortable openly discussing your health.</li>
        <li>Private settings include: a private room at home, a parked car, or a private room in a friend’s home. You may not attend from work or while driving.</li>
        <li>No Zoom filters, nicknames, or numbers in your display name. You must appear live on camera, upright, and fully dressed.</li>
        <li>No eating, drinking, smoking, or talking to others during the session. No public spaces such as stores or parks.</li>
        <li>Logging in more than 15 minutes late will result in an absence. Technical issues will be addressed by the facilitator as needed.</li>
        <li>Payment must be made before the session begins.</li>
      </ul>

      <h6>Before the Start of Your Appointment</h6>
      <ul>
        <li>Ensure good lighting and steady camera placement at eye level.</li>
        <li>Reduce background noise and close other applications to maintain connection quality.</li>
      </ul>

      <h6>Preparing for a Virtual Visit</h6>
      <ul>
        <li>Use a smartphone, computer, or tablet with internet and video capability.</li>
        <li>Dress appropriately and remain upright throughout the session.</li>
        <li>Find a private space, stay focused, and be patient if sessions run behind.</li>
        <li>Have any questions or concerns ready to discuss during your session.</li>
        <li>Clients are not allowed to miss more than three (3) group sessions.</li>
      </ul>

      <p><em>By signing below, you agree that you fully understand and agree to these terms and rules for the DWAG LLC program.</em></p>
    </div>

    <div class="form-check mb-4">
      <input class="form-check-input" type="checkbox" id="agree_telehealth" name="agree_telehealth" required>
      <label class="form-check-label" for="agree_telehealth">
        I Agree
      </label>
    </div>

    <!-- 9A-3: Final Acknowledgement -->
    <h5 class="mt-4">Acknowledgement of Review and Agreement</h5>
    <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 200px; background-color: #f8f9fa;">
      <p>By clicking Submit, I acknowledge the information on this form has been reviewed with me and I understand and agree to the conditions of the Dutch-Will Assessment Group LLC outlined above. I also acknowledge that I have received a copy of this agreement. Failure to comply with these agreements and rules may result in termination from the program and, if court-ordered, legal consequences.</p>
    </div>

    <div class="form-check mb-4">
      <input class="form-check-input" type="checkbox" id="agree_acknowledgement" name="agree_acknowledgement" required>
      <label class="form-check-label" for="agree_acknowledgement">
        I Agree
      </label>
    </div>
    ...
    <div class="mb-3">
      ...
      <input type="text" id="counselor" name="counselor" class="form-control" value="Brenda Riley">
    </div>
