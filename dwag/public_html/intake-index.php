...
$search=$_GET['search']??'';
$f_verified = $_GET['f_verified'] ?? 'all';   // all | 1 | 0
$f_imported = $_GET['f_imported'] ?? 'all';   // all | 1 | 0
if (!in_array($f_verified, ['all','1','0'], true))  $f_verified='all';
if (!in_array($f_imported,['all','1','0'], true))   $f_imported='all';
...
$allowed=['signature_date','first_name','last_name','email','phone_cell',
          ...
          'intake_date','referral_type_id','program_id'];
...
$url_prefix='search='.urlencode($search)
          ...
          .'&f_imported='.$f_imported
          .'&sort='.$toggle_sort;


$REF_MAP = [0=>'Other',1=>'Probation',2=>'Parole',3=>'Pretrial',4=>'CPS',5=>'Attorney'];
function ref_label(?int $id): string {
  return $GLOBALS['REF_MAP'][$id ?? -1] ?? 'Other';
}
...
/* main sql */
$conds = [];
if ($program_id) $conds[] = "program_id=".(int)$program_id;

/* verified/imported filters */
if ($f_verified !== 'all') $conds[] = "staff_verified=".( $f_verified==='1' ? '1' : '0' );
if ($f_imported !== 'all') $conds[] = "imported_to_client=".( $f_imported==='1' ? '1' : '0' );

/* search: text + referral label → id */
$ref_id_search = null;
if ($search!==''){
   $s_low = strtolower($search);
   foreach ($REF_MAP as $rid=>$lbl) {
      if (strpos(strtolower($lbl), $s_low) !== false) { $ref_id_search = (int)$rid; break; }
    }

   $esc = mysqli_real_escape_string($link,$search);
   $conds[] = "(".
      "CONCAT_WS(' ',first_name,last_name,email,phone_cell) LIKE '%$esc%' OR ".
      "id_number LIKE '%$esc%' OR ".
      "(address_street LIKE '%$esc%' OR address_city LIKE '%$esc%' OR address_state LIKE '%$esc%' OR address_zip LIKE '%$esc%') OR ".
      "referring_officer_name LIKE '%$esc%' OR referring_officer_email LIKE '%$esc%' OR ".
      "counselor_name LIKE '%$esc%' OR chosen_group_time LIKE '%$esc%'".
      ($ref_id_search!==null ? " OR referral_type_id=".$ref_id_search : "").
   ")";
}
...
$sql="SELECT
        intake_id,
        signature_date,
        DATE_FORMAT(signature_date,'%m/%d/%Y %h:%i %p') AS created_fmt,
        (signature_date >= DATE_SUB(NOW(), INTERVAL 7 DAY)) AS is_new,
        first_name,last_name,date_of_birth,email,phone_cell,
        packet_complete,staff_verified,imported_to_client,
        intake_date,referral_type_id,chosen_group_time
      FROM intake_packet".$where;
...
<form class="mb-3">
 ...
 <div class="form-row">
  <div class="col-md-3">
    ...
    <input type="text" name="search" class="form-control" placeholder="name / email / phone / referral" value="<?=h($search)?>">
  </div>
  <div class="col-md-2">
    <small class="text-muted">Verified</small>
    <select name="f_verified" class="form-control">
      <option value="all" <?= $f_verified==='all'?'selected':'' ?>>All</option>
      <option value="1"   <?= $f_verified==='1'  ?'selected':'' ?>>Yes</option>
      <option value="0"   <?= $f_verified==='0'  ?'selected':'' ?>>No</option>
    </select>
  </div>
  <div class="col-md-2">
    <small class="text-muted">Imported</small>
    <select name="f_imported" class="form-control">
      <option value="all" <?= $f_imported==='all'?'selected':'' ?>>All</option>
      <option value="1"   <?= $f_imported==='1'  ?'selected':'' ?>>Yes</option>
      <option value="0"   <?= $f_imported==='0'  ?'selected':'' ?>>No</option>
    </select>
  </div>
...
<thead><tr>
 ...
 <th><a href="?<?=$url_prefix?>&order=intake_date">Intake Date</a></th>
 <th><a href="?<?=$url_prefix?>&order=referral_type_id">Referral</a></th>
 <th>Group Choice</th>
...
<tr<?=$dup?' class="dup-row"':''?>>
 ...
 <td><?=h($r['intake_date']??'')?></td>
 <td><?=h(ref_label(isset($r['referral_type_id'])?(int)$r['referral_type_id']:null))?></td>
 <td><?=h($r['chosen_group_time']??'')?></td>
...
<?php endwhile; else: ?>
<tr><td colspan="13" class="text-center">No packets found</td></tr>
