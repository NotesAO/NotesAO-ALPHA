...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $NON_EDIT = [
      'intake_id','created_at','verified_at','verified_by',
      'packet_complete','staff_verified','imported_to_client','imported_client_id'
    ];

    // whitelist to real columns
    $colsInTbl = [];
    if ($d = $link->query("DESCRIBE intake_packet")) {
        while ($c = $d->fetch_assoc()) $colsInTbl[$c['Field']] = true;
        $d->close();
    }

    $sets = []; $vals = []; $types = '';
    foreach ($_POST as $k=>$v) {
        if ($k==='id' || $k==='csrf') continue;
        if (!isset($colsInTbl[$k])) continue;
        if (in_array($k,$NON_EDIT,true)) continue;
    ...
    if (!$sets) { header("Location: intake-review.php?id=$id"); exit; }
...
/* ==== 9a === */
'__h:Payment' => [],
'income_bracket'    => ['Income Bracket','text'],
'session_fee_cents' => ['Per-session Fee (cents)','number'],
...
'consent_telehealth'       => ['Consent – Telehealth','yesno'],
'consent_acknowledgement'  => ['Consent – Acknowledgement','yesno'],
...
/* ───────── 5b.  Auto-discover leftover columns ───────── */
$explicit = array_filter(array_keys($fields), fn($k)=>strpos($k,'__h:')!==0);
$explicit = array_flip($explicit);

$colMeta = [];  // name => ['label','type',opts]
if ($desc = $link->query("DESCRIBE intake_packet")) {
  while ($c = $desc->fetch_assoc()) {
    $name = $c['Field']; $type = strtolower($c['Type']);
    if (isset($explicit[$name])) continue;
    if (in_array($name, ['intake_id','created_at','verified_at','verified_by',
                         'packet_complete','staff_verified','imported_to_client','imported_client_id'], true)) continue;

    // Type inference
    $ftype = 'text';
    if (str_contains($type,'tinyint(1)')) $ftype='yesno';
    elseif (preg_match('/\b(date|datetime|timestamp)\b/',$type)) $ftype = (str_contains($type,'date') && !str_contains($type,'time'))?'date':'text';
    elseif (preg_match('/(description|details|reason|notes?)$/',$name)) $ftype='textarea';

    $colMeta[$name] = [ucwords(str_replace('_',' ',$name)),$ftype];
  }
  $desc->close();
}
if ($colMeta) {
  $fields['__h:Other (Auto-discovered)'] = [];
  foreach ($colMeta as $k=>$m) $fields[$k] = $m;
}
