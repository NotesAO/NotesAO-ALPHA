<?php
/*───────────────────────────────────────────────────────────
  client-victim-create.php – Add a NEW victim for the given client
───────────────────────────────────────────────────────────*/
...
/* 1) Resolve client_id and pull client header */
...
/* 2) Init form vars & error placeholders */
$name                 = '';
$relationship         = '';
$gender               = '';
$age                  = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // required
    if ($name === '')         $name_err         = 'Victim name is required.';
    if ($relationship === '') $relationship_err = 'Relationship is required.';

    // numeric
    if ($age !== '' && !ctype_digit($age))
                              $age_err         = 'Age must be a positive integer.';
    if ($children_under_18 !== '' && !ctype_digit($children_under_18))
    ...
    // if no errors, insert
    if (empty($name_err . $relationship_err . $age_err . $children_err . $zip_err . $email_err)) {
        $sql = "
          INSERT INTO victim
            (client_id,
             name,
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-8 mx-auto">
        ...
        <p class="text-muted">Complete all relevant fields, then submit.</p>
        ...
        <form method="post" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>">
          ...
          <div class="form-group">
            ...
            <select name="gender" class="form-control">
              ...
              <?php foreach(['male','female','non-binary','other'] as $opt): ?>
                <option value="<?= $opt ?>"
                  <?= $opt === $gender ? 'selected':'' ?>>
                  <?= ucfirst($opt) ?>
                </option>
          ...
          <div class="form-row">
            ...
            <div class="form-group col-md-6 align-self-end">
              <div class="form-check">
                <input type="checkbox" name="living_with_client"
                       id="reside" class="form-check-input"
