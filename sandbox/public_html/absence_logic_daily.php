...
echo "<b>Checking attendance for client " . $client_id ."</b><br>";;
        ...
        //  BEWARE TIMES are dangerous in this logic.

        // Looks back 4 weeks
        // Starting at first attendance)
        ...
        $stop_date = (new DateTime( "yesterday" ))->setTime(0,0,0,0);
        $weeks_ago = ((new DateTime( "yesterday" ))->setTime(0,0,0,0))->modify("-4 weeks");
        $date_idx = (new DateTime($start_date))->setTime(0,0,0,0);

        // Advance start date to yesterday's day of week
        while($date_idx->format("w") <> $stop_date->format("w")) {
            $date_idx->modify("+1 day");
        }

        // Only look back 4 weeks even if they started before that
...
echo "Analyze week starting " . $week_start->format("Y-m-d") . " attendance " . $record_count . " required " . $weekly_attendance ."<br>";
    ...
    function calculateAbsenceDay($start_day, $attendance_list, $absence_list, $row){
        ...
        while(!$dayFound && $count < 7) {
            ...
            if(in_array($absence_day->format("Y-m-d"), $attendance_list) ||
...
<head>
    <meta charset="UTF-8">
    <title>NotesAO - Absence Daily</title>
