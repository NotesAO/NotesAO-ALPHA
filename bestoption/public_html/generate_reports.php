<?php
include '/home/notesao/bestoption/public_html/auth.php';
...
// -------------------------------------------------------------------
// 1) Define variables (once) and normalize program
// -------------------------------------------------------------------
$clinic_folder = 'bestoption';

$BASE_DIR       = '/home/notesao/NotePro-Report-Generator';
$TEMPLATES_DIR  = $BASE_DIR . '/templates';   // templates ROOT (not clinic)
$base_dir       = $BASE_DIR;                  // legacy alias for rest of file
...
// normalize program to the 3 reporting buckets (BIPP unified)
function normalize_program_name($p) {
  $p = trim((string)$p);
  if ($p === '') return $p;
  $lp = strtolower($p);
  if (strpos($lp, 'bipp') !== false)      return 'BIPP';
  if (strpos($lp, 'anger') !== false)     return 'Anger Control';
  if (strpos($lp, 'substance') !== false) return 'Substance Abuse';
  return $p;
}
$program  = normalize_program_name($program);

// program code used by fallback (and later by Python)
if ($program === 'BIPP') {
  $progCode = 'BIPP';
} elseif ($program === 'Anger Control') {
  $progCode = 'AC';
} elseif ($program === 'Substance Abuse') {
  $progCode = 'SAE';
} else {
  $progCode = 'UNKNOWN';
}
...
// Only validate program for generation path; download_zip may not send it
if ($action !== 'download_zip') {
    $valid_programs = ['BIPP', 'Anger Control', 'Substance Abuse'];
...
// clinic-specific paths (unchanged)
...
$templates_dir                  = "$base_dir/templates/$clinic_folder"; // keep for now

// -------------------------------------------------------------------
// 2) Ensure directories exist
// -------------------------------------------------------------------

// e.g. /home/notesao/NotePro-Report-Generator/templates/bestoption
...
// If $program was NOT normalized earlier, normalize it now (but do not redeclare the function)
$program = normalize_program_name($program);

// program code (no function redeclare here)
if ($program === 'BIPP') {
    $progCode = 'BIPP';
} elseif ($program === 'Anger Control') {
    $progCode = 'AC';
...
// Program validation (report validation will happen after $script_paths is set)


// Normalize & dedupe incoming report labels
$reports = array_values(array_unique(array_map('strval', $reports)));



// -------------------------------------------------------------------
// Fallback template helpers (program name -> default folder; flat symlink)
// -------------------------------------------------------------------
function program_key_from_name($program) {
    static $map = array(
        'BIPP'            => 'BIPP', // unified BIPP
        'Anger Control'   => 'AC',
        'Substance Abuse' => 'SAE',  // SAE scripts/templates
    );
    return isset($map[$program]) ? $map[$program] : 'unknown';
}

/**
 * Ensure every required template file exists flat under:
 *   /home/notesao/NotePro-Report-Generator/templates/{clinic}/FILENAME.docx
 * If a file is missing, symlink it from:
 *   /home/notesao/NotePro-Report-Generator/templates/default/{PROG_CODE}/FILENAME.docx
 * where PROG_CODE is one of BIPP, AC, SAE.
 */
function ensure_fallback_templates($base_dir, $clinic_folder, $program) {
    $progKey   = program_key_from_name($program);
    if ($progKey === 'unknown') {
        error_log("Unknown program key for '$program'; skipping fallback templates");
        return;
    }

    $clinicRoot = $base_dir . "/templates/" . $clinic_folder;
    $defaultDir = $base_dir . "/templates/default/" . $progKey;

    if (!is_dir($clinicRoot)) {
        @mkdir($clinicRoot, 0755, true);
    }
    if (!is_dir($defaultDir)) {
        error_log("Default dir missing: $defaultDir");
        return;
    }

    // Link any missing .docx from default/<PROG_CODE>/ into clinic root
    foreach (glob($defaultDir . '/*.docx') as $src) {
        $dst = $clinicRoot . '/' . basename($src);
        if (!file_exists($dst)) {
            @symlink($src, $dst); // so Python finds expected filenames in the clinic folder
            error_log("FALLBACK: linked $dst -> $src");
        }
    }
}
...
if (!empty($client_name)) {
    /**
     * New parsing logic ─ last token = last_name,
     * everything before = first_name (allows “Joe Ray Vasquez” ➜
     * first_name = “Joe Ray”, last_name = “Vasquez”).
     */
    ...
    $last_name  = array_pop($parts);          // take final token
    $first_name = implode(' ', $parts);       // re-join the rest
    // Optional: normalise case/trim here if you like
    // $first_name = ucfirst(strtolower($first_name));

    // Lookup in client table
...
ensure_fallback_templates($base_dir, $clinic_folder, $program);
...
$script_paths = [
    "BIPP" => [
        ...
        "Victim Letters" => "$base_dir/BIPP_Victim_Letters_Script.py"
    ],
    "Anger Control" => [
    ...
    "Substance Abuse" => [
        "Completion Documents"   => "$base_dir/SAE_Completion_Documents_Script.py",
        "Entrance Notifications" => "$base_dir/SAE_Entrance_Notifications_Script.py",
        "Exit Notices"           => "$base_dir/SAE_Exit_Notices_Script.py",
        "Unexcused Absences"     => "$base_dir/SAE_Unexcused_Absences_Script.py",
        "Progress Reports (Stage of Change)"=> "$base_dir/SAE_Progress_Reports_Script.py",
        "Progress Reports (Curriculum)"=> "$base_dir/SAE_Progress_Reports_Script.py"
    ]
];

// Validate selected reports against what this program actually supports
$allowed = array_keys($script_paths[$program] ?? []);
if (empty($allowed)) {
    echo json_encode(['status' => 'error', 'message' => "No scripts configured for program: $program"]);
    unlink($converted_csv_path);
...
foreach ($reports as $report) {
    if (!in_array($report, $allowed, true)) {
        echo json_encode(['status' => 'error', 'message' => "Invalid report type for $program: $report"]);
        unlink($converted_csv_path);
        if ($filtered_csv_path !== $converted_csv_path) { @unlink($filtered_csv_path); }
        exit;
    }
...
foreach ($reports as $report_type) {
    ...
    // pass root templates dir + program code so Python can resolve defaults
    $TEMPLATES_DIR = $base_dir . '/templates';
    $envPrefix =
    'NP_TEMPLATES_DIR=' . escapeshellarg($TEMPLATES_DIR) . ' ' .
    'NP_PROGRAM_CODE=' . escapeshellarg($progCode) . ' ' .
    'NP_CLINIC_FOLDER=' . escapeshellarg($clinic_folder) . ' ';
    ...
    $cmd  = $envPrefix
        ...
        . ' --templates_dir ' . escapeshellarg($templates_dir)   // keep for now
    ...
    if ($report_type === "Progress Reports (Curriculum)") {
        $cmd .= ' --db_host ' . escapeshellarg('50.28.37.79')
            . ' --db_user ' . escapeshellarg('clinicnotepro_bestoption_app')
            . ' --db_pass ' . escapeshellarg('PF-m[T-+pF%g')
            . ' --db_name ' . escapeshellarg('clinicnotepro_bestoption');
    }
