<?php
/**
 * BestOption – BIPP Virtual Intake Packet
 * ------------------------------------
 * • GET  : render multi‑step form  (HTML follows this PHP section)
 * • POST : CSRF check → validate → INSERT into clinicnotepro_bestoption.intake_packet
...
ob_start();
session_start();
csrf_token(); // ensure $_SESSION['csrf'] is initialized for this request
...
function csrf_check(): void
{
    ...
    $known = $_SESSION['csrf']     ?? '';
    if ($form === '' || $known === '' || !hash_equals($known, $form)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $program_sel = postv('program_select');   // 'bipp', 'anger', 'su'
    $gender      = postv('gender_id');        // still captured for storage

    switch ($program_sel) {
        case 'anger': $program_id = 1; break;           // Anger Control
        case 'su':    $program_id = 4; break;           // Substance Use
        default:      // BIPP (or anything unknown falls back to BIPP)
            $program_id = ($gender === '3') ? 3 : 2;    // 3 = female, 2 = male
    }
    ...
    $fields = [
        ...
        'program_select'        => $program_sel,
        'is_readmit'            => postb('is_readmit'),
        ...
        'agree_ua_policy'         => postb('agree_ua_policy'),
        'ack_relapse_plan'        => postb('ack_relapse_plan'),
        'optin_client_survey'     => postb('optin_client_survey'),
        'agree_bipp_guidelines'   => postb('agree_bipp_guidelines'),

        /* Outpatient + Telehealth + Payment + ROI */
        'bill_of_rights_outpatient_ack' => postb('bill_of_rights_outpatient_ack'),
        'grievance_outpatient_ack'      => postb('grievance_outpatient_ack'),
        'telehealth_consent'            => postb('telehealth_consent'),
        'telehealth_no_recording'       => postb('telehealth_no_recording'),
        'telehealth_private_space'      => postb('telehealth_private_space'),
        'telehealth_alt_contact_ok'     => postb('telehealth_alt_contact_ok'),
        'payment_policy_ack'            => postb('payment_policy_ack'),

        'roi_enable'                    => postb('roi_enable'),
        'roi_recipient_name'            => postv('roi_recipient_name'),
        'roi_address'                   => postv('roi_address'),
        'roi_city'                      => postv('roi_city'),
        'roi_state'                     => postv('roi_state'),
        'roi_zip'                       => postv('roi_zip'),
        'roi_phone'                     => postv('roi_phone'),
        'roi_fax'                       => postv('roi_fax'),
        'roi_authorize_demographics'    => postb('roi_authorize_demographics'),
        'roi_authorize_correspondence'  => postb('roi_authorize_correspondence'),
        'roi_authorize_services'        => postb('roi_authorize_services'),
        'roi_authorize_intake_summary'  => postb('roi_authorize_intake_summary'),
        'roi_authorize_attendance'      => postb('roi_authorize_attendance'),
        'roi_authorize_progress_notes'  => postb('roi_authorize_progress_notes'),
        'roi_authorize_drug_alcohol_results' => postb('roi_authorize_drug_alcohol_results'),
        'roi_authorize_other_text'      => postv('roi_authorize_other_text'),
        'roi_purpose_text'              => postv('roi_purpose_text'),
        'roi_expiration_text'           => postv('roi_expiration_text'),
        'roi_signed_name'               => postv('roi_signed_name'),
        'roi_signed_at'                 => postv('roi_signed_at'),
    ...
    $headers  = "From: reporting@tbo.notesao.com\r\n";
    $headers .= "Reply-To: " . postv('email') . "\r\n";   // nice but optional
    $headers .= "X-Mailer: PHP/" . PHP_VERSION;

    @mail(
        ADMIN_ALERT_EMAIL,
        "The Best Option has received a new Intake Packet for $fname $lname",
        "A new online intake packet was submitted.\n\nView pending & submitted packets:\nhttps://{$_SERVER['HTTP_HOST']}/intake-index.php",
        $headers,
        '-freporting@tbo.notesao.com'            // ← some MTAs (esp. cPanel) require this
    );
...
if (!empty($_SESSION['show_thank_you_once'])) {
    ...
    <div class="jumbotron bg-white text-center shadow-sm py-4 mb-4">
      <a href="https://thebestoptionsa.com/" target="_blank" rel="noopener">
        <img src="bestoptionlogo.png" alt="The Best Option"
    ...
    <div class="card shadow-sm">
      <div class="card-body text-center p-5">
        ...
        <a href="https://thebestoptionsa.com/" class="btn btn-primary btn-lg">
          The Best Option
        </a>
      </div>
    </div>
...
<head>
<meta charset="utf-8">
<title>The Best Option | Intake</title>
...
<!-- Jumbotron / Header Section -->
    <div class="jumbotron bg-white text-center shadow-sm py-4">
      <a href="https://thebestoptionsa.com/" target="_blank">
        <!-- Responsive Image -->
          <img 
              src="bestoptionlogo.png" 
              alt="The Best Option" 
...
<form method="post">
  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(),ENT_QUOTES) ?>">
    ...
    <ol style="margin-left:1.25rem">
        ...
        <li><strong>Complete All Required Fields:</strong> most fields are marked as <span style="color:#d2302c;font-weight:700">required</span> and must be filled out. You will not be able to submit the form, move forward, or begin the BIPP, Anger Control, or Substance Use programs until the form is fully completed.</li>
    ...
    <ul>
        ...
        <li>The form will be added to your chart.</li>
    </ul>

    <p>If you have questions while filling out the form or encounter any technical difficulties, please contact our support team at: <strong>(210) 265-1133</strong>.</p>
  ...
  <fieldset class="step">
    ...
    <div class="row">
      ...
      <div class="col">
        ...
        <select id="program_select" name="program_select" required>
          <option value="">-- Select Program --</option>
          <option value="bipp">BIPP – Battering Intervention</option>
          <option value="anger">Anger Control</option>
          <option value="su">Substance Use</option>
        </select>
      </div>
    </div>

    <div class="row mt-2" id="readmit_block" style="display:none;">
      <div class="col">
        <label class="required">Is this a re-admit?</label>
        <select id="is_readmit" name="is_readmit">
    ...
    outside The Best Option programs. This right applies with the following exceptions:</p>
    ...
    <ul>
        ...
        <li><strong>Media involvement:</strong> Any media contact arranged by The Best Option program
            will include the presence of a Best Option employee to protect victim confidentiality.</li>
    </ul>

    <p><strong>We ask that you keep confidential information you may learn about other clients who are
    receiving services from The Best Option programs.</strong></p>

    <p><strong>The Best Option requires facilitators and participants to:</strong></p>
    <ul>
    ...
    <ul>
        ...
        <li>If unresolved, report concerns to your counselor's immediate supervisor, Executive Director
            Helen Bennett-Lopez, at 210-265-1133.</li>
    ...
    I accept these terms as a condition of participation in The Best Option programs.</em></p>
    </div>
  ...
  <fieldset class="step" id="step-victim">
    ...
    <div id="victim-info-block" class="d-none mt-3">
      ...
      <div class="row no-gap mt-3" data-show-programs="bipp">
        <div class="col-md-6">
          <label class="required" for="live_with_victim">Will you be living with the victim while attending BIPP?</label>
          <select id="live_with_victim" name="live_with_victim" class="form-select" data-required-when-visible="1">
  ...
  <fieldset class="step" id="step-client-consents">
    ...
    <div class="mb-4">
      ...
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        ...
        <p>I understand that I may revoke this consent at any time and that my request for revocation must be in writing. If not earlier revoked, this consent for disclosure of information shall expire 1 year after my completion of or termination from The Best Option programs.</p>
        <p>I understand my right to confidentiality. I further understand that this consent form gives The Best Option permission to share confidential information about me in the way described above.</p>
        <p>I understand that Victim will be contacted by the Victim Advocate and offered counseling services. She/He will be provided enrollment, completion, or termination information from The Best Option program.</p>
        <p>Release of information is voluntary; I understand I have a right to refuse The Best Option's request for this disclosure.</p>
        <p>The Best Option programs reserves the right to dismiss any client who refuses to meet the provisions of The Texas Department of Criminal Justice-Community Justice Assistance Division and Texas Council on Family Violence Battering Intervention & Prevention Project guidelines.</p>
      ...
      <div class="form-check">
        ...
        <label class="form-check-label" for="agree_disclosure">
          By checking "I Agree", I confirm that I have read, understood, and agree to abide by the terms and conditions outlined above. I acknowledge my rights and responsibilities as described, and I accept these terms as a condition of participation in The Best Option program.
        </label>
      </div>
    </div>

    <!-- Section: The Best Option Agreement -->
    <div class="mb-4">
      <label class="form-label fw-bold">The Best Option Program Agreement</label>
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        <p>You need to make sure that you are on time for your scheduled group time. You should be at the office 30 minutes prior to your scheduled group time. This gives you time to make your group payment and go to the restroom so you will not be late for your group.</p>

        <p>You are required to pay the $25.00 fee before your scheduled group. This is paid by calling the office at 210.265.1133, Monday ‑ Friday from 9 am to 4 pm, or arrive 30 minutes prior to group starting to make your payment using a debit/credit card.</p>

        <p>Note a $25.00 no show fee will be charged for ALL missed group sessions. No exceptions.</p>

        <p>Invoices are ran the day after your scheduled group. You may leave a debit/credit card on file to be automatically withdrawn. You need to make sure that the debit/credit card on file is set to be your default card for payment. It will be your responsibility to make sure that your payment cleared.</p>

        <p>If you have a balance due to non‑payment due to your payment not clearing, you may log into your client portal account at https://tbo.notesao.com/clientportal.php/ to view and pay your balance.</p>

        <p>I authorize The Best Option, LLC to charge my credit/debit/health account card for professional services. The fee for service for BIPP Assessment and Orientation is $30 and the fee per Group session will be $25. I am aware that the $30 payment is required for missed BIPP Assessment and Orientation appointments and the $25 payment is required for missed Group appointments as well.</p>
    ...
    <div class="mb-4">
      ...
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        ...
        <p>The Best Option program requires me to disable any devices that could collect information from the environment, such as Google Home Assistant, Amazon Alexa, or Apple Siri, and I agree that I will not record nor take screenshots of the group. The Best Option requires me to be in a private space and not in any public area such as a park, yard, or open area; other people not in the group should not be exposed to the content nor hear or observe the group.</p>
      ...
      <div class="form-check">
        ...
        <label class="form-check-label" for="agree_responsibility">
          By checking "I Agree", I confirm that I have read, understood, and agree to abide by the terms and conditions outlined above. I acknowledge my rights and responsibilities as described, and I accept these terms as a condition of participation in The Best Option programs.
        </label>
      </div>
    </div>
    ...
    <div class="mb-4">
      ...
      <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 240px; background-color: #f8f9fa;">
        <p>I have received a copy of the "Policy for Clients" for The Best Option programs. I understand my rights and responsibilities, and I agree to enter The Best Option programs.</p>
        <p>I understand that in accordance with Guideline 31 of the Texas Department of Criminal Justice-Community Justice Assistance Division and Texas Council on Family Violence Battering Intervention Prevention Program guidelines, I am being provided a written agreement that clearly delineates the obligation of The Best Option program to the client. I understand that The Best Option shall:</p>
        <ul>
        ...
        <ul>
          ...
          <li>The Best Option programs client violates any of the BIPP rules.</li>
        </ul>
        ...
        <p>Clients have the right to seek other resources outside of The Best Option programs, and when possible, The Best Option staff will provide or make a referral.</p>
        ...
        <p>The Best Option program will provide batterers at the time of assessment (intake) with a copy of the circumstances under which they can be terminated before completion.</p>
        <p>I have read and understand the above statements and voluntarily enter into counseling services from the staff of The Best Option programs - by entering my name below. (By clicking SUBMIT, I hereby confirm the above information to the best of my knowledge is correct and true, with no misleading or false content in accordance with Texas Perjury Statute, Sec. 37.02 (a) (2) Chapter 32, Civil Practice and Remedies Code.)</p>
      </div>
      <div class="form-check">
    ...
    <!-- UA Policy (all programs) ----------------------------------- -->
    <div class="policy">
      <strong>Urinalysis (UA) Policy</strong><br>
      A urine drug screen will be collected at the time of screening / intake and
      may be required at any other time at the request of The Best Option staff to
      verify compliance with treatment requirements.
      <ul style="margin-top:.5rem">
        <li>You have <strong>two (2) hours</strong> from notification to appear for testing.</li>
        <li>Refusal to test, failure to appear, or a positive result is non‑compliance and
            may trigger sanctions such as additional assignments, increased attendance
            requirements, or <em>unsuccessful discharge</em>.</li>
        <li>One of the explicit reasons for discharge is a positive urine drug screen while in treatment.</li>
      </ul>
    </div>

    <div class="form-check mb-3">
      <input class="form-check-input" type="checkbox" id="agree_ua_policy" name="agree_ua_policy" required>
      <label class="form-check-label" for="agree_ua_policy">
        I have read and understand the UA policy.
      </label>
    </div>

    <div class="su-only" data-show-programs="su">
      <div class="policy">
        <strong>Relapse‑Prevention Plan Commitment</strong><br>
        You must complete and submit a written <em>Relapse‑Prevention Recovery Plan</em>
        <u>before your 4<sup>th</sup> group session</u>.  The plan must address the
        following (per TBO’s official worksheet):
        <ul style="margin-top:.5rem">
          <li>High‑risk situations &amp; coping strategies</li>
          <li>Irrational thoughts, emotional &amp; physical triggers</li>
          <li>Concrete steps you will take if a relapse occurs</li>
          <li>Support groups you will attend and your reason for each</li>
          <li>Your personal relapse triggers and accountability tactics</li>
          <li>How you will handle peer pressure, anger and leisure time</li>
        </ul>
        Failure to turn in a complete plan by Week‑4 can delay progress or result
        in discharge.
      </div>

      <div class="form-check mb-3">
        <input class="form-check-input" type="checkbox" id="ack_relapse_plan" name="ack_relapse_plan" data-required-when-visible="1">
        <label class="form-check-label" for="ack_relapse_plan">
          I have read and completed my Relapse‑Prevention Plan.
        </label>
      </div>
    </div>
    ...
    <div class="su-only" data-show-programs="su">
      <div class="form-check mb-3">
        <input class="form-check-input" type="checkbox" id="optin_client_survey" name="optin_client_survey">
        <label class="form-check-label" for="optin_client_survey">
          I agree to receive a follow‑up Client Satisfaction Survey by e‑mail.
        </label>
      </div>
    </div>

      <div class="policy">
        <strong>BIPP Group Requirements and Guidelines</strong><br>
        <p class="mb-1"><strong>Be on time.</strong> Arrive 30 minutes prior to your scheduled group time to make your payment and use the restroom so you will not be late.</p>
        <p class="mb-1"><strong>Rules:</strong></p>
        <ol class="mb-2">
          <li>Arrive 30 minutes prior to your group starting.</li>
          <li>Actively participate.</li>
          <li>Turn in all weekly check-ins and homework on time.</li>
        </ol>
        <p class="mb-1"><strong>Payment reminders:</strong> Pre-pay $25 before group. A $25 no-show fee applies. Invoices run the day after group. You may keep a card on file; ensure payment clears. Portal: <u>https://tbo.notesao.com/clientportal.php/</u>.</p>
        <p class="mb-0"><strong>Possible termination reasons:</strong> Continued abuse (especially physical violence), non-attendance, and non-compliance with written conditions and program rules.</p>
      </div>

      <div class="form-check mb-3" data-show-programs="bipp">
        <input class="form-check-input" type="checkbox" id="agree_bipp_guidelines" name="agree_bipp_guidelines" data-required-when-visible="1">
        <label class="form-check-label" for="agree_bipp_guidelines">
          I agree to follow the BIPP Requirements &amp; Guidelines.
        </label>
      </div>
    ...
    <!-- ================================================================== -->
  <!--  8b. OUTPATIENT CLIENT BILL OF RIGHTS                               -->
  <!-- ================================================================== -->
  <fieldset class="step">
    <legend>8b&nbsp;&nbsp;Outpatient Client Bill of Rights</legend>
    <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 360px; background-color: #f8f9fa;">
      <h6 class="mb-2">Client Bill of Rights for Outpatient Treatment</h6>
      <p>The Best Option shall respect, protect, implement and enforce each client right required to be contained in the facility's Client Bill of Rights.</p>
      <ol class="mb-0">
        <li>You have the right to accept or refuse treatment after receiving this explanation.</li>
        <li>If you agree to treatment or medication, you have the right to change your mind at any time (unless specifically restricted by law).</li>
        <li>You have the right to a humane environment that provides reasonable protection from harm and appropriate privacy for your personal needs.</li>
        <li>You have the right to be free from abuse, neglect, and exploitation.</li>
        <li>You have the right to be treated with dignity and respect.</li>
        <li>You have the right to appropriate treatment in the least restrictive setting available that meets your needs.</li>
        <li>You have the right to be told about the program’s rules and regulations before you are admitted, including rules related to restraints and seclusion; your legally authorized representative, if any, shall also be notified.</li>
        <li>You have the right to be told before admission: (a) the condition to be treated; (b) the proposed treatment; (c) the risks, benefits, and side effects of all proposed treatment and medication; (d) the probable health and mental-health consequences of refusing treatment; (e) other treatments that are available and which ones, if any, might be appropriate for you; and (f) the expected length of stay.</li>
        <li>You have the right to a treatment plan designed to meet your needs, and the right to take part in developing that plan.</li>
        <li>You have the right to meet with staff to review and update the plan on a regular basis.</li>
        <li>You have the right to refuse to take part in research without affecting your regular care.</li>
        <li>You have the right not to receive unnecessary or excessive medication.</li>
        <li>You have the right to have information about you kept private and to be told about the times when the information can be released without your permission.</li>
        <li>You have the right to be told in advance of all estimated charges and any limitations on the length of services of which the facility is aware.</li>
        <li>You have the right to receive an explanation of your treatment or your rights if you have questions while you are in treatment.</li>
        <li>You have the right to make a complaint and receive a fair response from the facility within a reasonable amount of time.</li>
        <li>You have the right to complain directly to the Texas Health and Human Services Commission at any reasonable time.</li>
        <li>You have the right to get a copy of these rights before you are admitted, including the address and phone number of the Texas Health and Human Services Commission.</li>
        <li>You have the right to have your rights explained to you in simple terms, in a way you can understand, within 24 hours of being admitted.</li>
      </ol>
    </div>

    <div class="form-check mb-3">
      <input class="form-check-input" type="checkbox" id="bill_of_rights_outpatient_ack" name="bill_of_rights_outpatient_ack" required>
      <label class="form-check-label" for="bill_of_rights_outpatient_ack">I acknowledge the Outpatient Client Bill of Rights.</label>
    </div>
    <div class="nav-btns">
  ...
  <!-- ================================================================== -->
  <!--  8c. OUTPATIENT CLIENT GRIEVANCE PROCEDURE                          -->
  <!-- ================================================================== -->
  <fieldset class="step">
    <legend>8c&nbsp;&nbsp;Outpatient Client Grievance Procedure</legend>
    <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 360px; background-color: #f8f9fa;">
      <h6 class="mb-2">Client Grievance Procedure</h6>
      <p>Listed below is The Best Option Grievance Procedure. Your signature acknowledges that you have read and understand the procedure.</p>
      <ol class="mb-2">
        <li>File a grievance about any violation of client rights or HHSC rules.</li>
        <li>Submit a grievance in writing and get help writing it if you are unable to write.</li>
        <li>You may request writing materials, postage, and access to a telephone for the purpose of filing a grievance.</li>
        <li>You may submit a grievance directly at The Best Option, 3700 Fredericksburg Road, Suite 137, San Antonio, Texas 78201.</li>
        <li>You may also submit the grievance to the Health and Human Services Commission, Complaint and Incident Intake, Mail Code E-249, P.O. Box 149030, Austin, Texas 78714-9030. Complaint hotline: 1-800-458-9858 (Option 6). Email: cii.sa@hhs.texas.gov. Fax: 833-709-5735.</li>
      </ol>
      <p class="mb-1"><strong>Facility response:</strong></p>
      <ol class="mb-0" start="6">
        <li>Evaluate the grievance thoroughly and objectively, obtaining additional information as needed.</li>
        <li>Provide a written response to the client within seven (7) days of receiving the grievance.</li>
        <li>Take action to resolve all grievances promptly and fairly.</li>
        <li>Document all grievances, including the final disposition, and keep the documentation in a central file.</li>
        <li>The facility shall not retaliate against clients who exercise their rights or file a grievance, nor restrict, discourage, or interfere with client communication with an attorney or with HHSC for the purposes of filing a grievance.</li>
      </ol>
    </div>

    <div class="form-check mb-3">
      <input class="form-check-input" type="checkbox" id="grievance_outpatient_ack" name="grievance_outpatient_ack" required>
      <label class="form-check-label" for="grievance_outpatient_ack">I acknowledge the Outpatient Grievance Procedure.</label>
    </div>
    <div class="nav-btns">
      <button type="button" class="btn btn-secondary prev">Back</button>
      <button type="button" class="btn btn-primary next">Next</button>
  ...
  <!-- ================================================================== -->
  <!--  8d. TELEHEALTH TREATMENT CONSENT                                  -->
  <!-- ================================================================== -->
  <fieldset class="step">
    <legend>8d&nbsp;&nbsp;Telehealth Treatment Consent</legend>
    <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 360px; background-color: #f8f9fa;">
      <h6 class="mb-2">Information and Informed Consent for Telemental Health Treatment</h6>
      <p>Telemental health is live two-way audio and video electronic communications that allows therapists and clients to meet outside of a physical office setting.</p>
      <ol class="mb-0">
        <li>Telemental health services are voluntary and you can withdraw consent at any time.</li>
        <li>No sessions will be recorded or photographed. You agree not to record or take screenshots.</li>
        <li>Disable any device that could collect environmental information, such as Google Home, Amazon Alexa, or Apple Siri.</li>
        <li>Attend and participate in a private space and remain in one location during sessions. Do not require your partner or children to relocate from the home to attend.</li>
        <li>Arrange supervision for children so sessions are not interrupted and confidentiality is maintained.</li>
        <li>Laws protecting privacy and confidentiality apply. Information identifying you will not be disclosed to other entities without your consent.</li>
        <li>Telehealth uses secure communication systems, but no internet communication is 100% guaranteed secure. The practice is not responsible if an outside party bypasses security measures.</li>
        <li>There are potential risks, including interruptions, unauthorized access, and technical difficulties.</li>
        <li>You or your therapist may discontinue sessions at any time if video technology is inadequate.</li>
        <li>In an emergency during a session, your therapist may call emergency services and/or your emergency contact.</li>
        <li>This consent is in addition to the Notice of Privacy Practices and Consent to Treatment. All office policies apply.</li>
        <li>If the video connection drops, you will have a backup phone line or a plan made in advance for re-contact.</li>
        <li>A “no show” or late fee will be charged if you miss or do not cancel within 24 hours. A payment method will be established before the first session.</li>
        <li>Your therapist will advise what platform to use and will establish the video session.</li>
      </ol>
    </div>

    <div class="form-check mb-2">
      <input class="form-check-input" type="checkbox" id="telehealth_consent" name="telehealth_consent" required>
      <label class="form-check-label" for="telehealth_consent">I consent to telehealth services.</label>
    </div>

    <!-- Optional affirmations mapped to nullable booleans -->
    <div class="form-check"><input class="form-check-input" type="checkbox" id="telehealth_no_recording" name="telehealth_no_recording"><label class="form-check-label" for="telehealth_no_recording">I will not record sessions.</label></div>
    <div class="form-check"><input class="form-check-input" type="checkbox" id="telehealth_private_space" name="telehealth_private_space"><label class="form-check-label" for="telehealth_private_space">I will use a private space when possible.</label></div>
    <div class="form-check mb-3"><input class="form-check-input" type="checkbox" id="telehealth_alt_contact_ok" name="telehealth_alt_contact_ok"><label class="form-check-label" for="telehealth_alt_contact_ok">Provider may contact me by phone/text if a session drops.</label></div>
  ...
  <!-- ================================================================== -->
  <!--  8e. PAYMENT POLICY ACKNOWLEDGMENT                                  -->
  <!-- ================================================================== -->
  <fieldset class="step">
    <legend>8e&nbsp;&nbsp;Payment Policy Acknowledgment</legend>
    <div class="border p-3 mb-2 rounded overflow-auto" style="max-height: 360px; background-color: #f8f9fa;">
      <h6 class="mb-2">Payment Policy</h6>
      <ol class="mb-2">
        <li>You are required to pre-pay the $25.00 group fee before your scheduled group. Pay by calling 210-265-1133, Monday–Friday 9 am–3 pm.</li>
        <li>A $25.00 no-show fee is charged for all missed group sessions.</li>
        <li>Invoices are run the day after your scheduled group. You may leave a card on file for automatic withdrawal. Ensure the card on file is set as default and that payment clears.</li>
        <li>You can view and pay any balance at <u>https://tbo.notesao.com/clientportal.php/</u>.</li>
      </ol>
      <p class="mb-1"><strong>Card payment consent summary:</strong> You authorize The Best Option, LLC to charge your card for services. BIPP Assessment and Orientation fee is $30. Group session fee is $25. The $30 fee applies to missed Assessment/Orientation and the $25 fee applies to missed group appointments.</p>
    </div>

    <div class="form-check mb-3">
      <input class="form-check-input" type="checkbox" id="payment_policy_ack" name="payment_policy_ack" required>
      <label class="form-check-label" for="payment_policy_ack">I acknowledge the payment policy.</label>
    </div>
    <div class="nav-btns">
      <button type="button" class="btn btn-secondary prev">Back</button>
      <button type="button" class="btn btn-primary next">Next</button>
  ...
  <!-- ================================================================== -->
  <!--  8f. CONSENT FOR RELEASE OF INFORMATION (optional)                  -->
  <!-- ================================================================== -->
  <fieldset class="step">
    <legend>8f&nbsp;&nbsp;Consent for Release of Confidential Information (Optional)</legend>

    <div class="form-check mb-3">
      <input class="form-check-input" type="checkbox" id="roi_enable" name="roi_enable">
      <label class="form-check-label" for="roi_enable">Enable a Release of Information for a specific recipient</label>
    </div>

    <div id="roi_block" class="border p-3 rounded" style="display:none; background-color:#f8f9fa;">
      <div class="row mb-2">
        <div class="col-md-6"><label>Recipient Name</label><input class="form-control" name="roi_recipient_name"></div>
        <div class="col-md-6"><label>Address</label><input class="form-control" name="roi_address"></div>
      </div>
      <div class="row mb-2">
        <div class="col-md-4"><label>City</label><input class="form-control" name="roi_city"></div>
        <div class="col-md-2"><label>State</label><input class="form-control" maxlength="2" name="roi_state"></div>
        <div class="col-md-3"><label>Zip</label><input class="form-control" maxlength="10" name="roi_zip"></div>
        <div class="col-md-3"><label>Phone</label><input class="form-control" name="roi_phone"></div>
      </div>
      <div class="row mb-2">
        <div class="col-md-4"><label>Fax</label><input class="form-control" name="roi_fax"></div>
      </div>

      <label class="fw-bold mt-2">Information Authorized to Release</label>
      <div class="row">
        <div class="col-md-6">
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_demographics" id="roi_demographics"><label class="form-check-label" for="roi_demographics">Demographics</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_correspondence" id="roi_correspondence"><label class="form-check-label" for="roi_correspondence">Correspondence</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_services" id="roi_services"><label class="form-check-label" for="roi_services">Services and dates</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_intake_summary" id="roi_intake"><label class="form-check-label" for="roi_intake">Intake summary</label></div>
        </div>
        <div class="col-md-6">
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_attendance" id="roi_attendance"><label class="form-check-label" for="roi_attendance">Attendance</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_progress_notes" id="roi_progress"><label class="form-check-label" for="roi_progress">Progress notes</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" name="roi_authorize_drug_alcohol_results" id="roi_ua"><label class="form-check-label" for="roi_ua">Drug/Alcohol test results</label></div>
        </div>
      </div>
      <div class="mt-2">
        <label>Other (specify)</label>
        <input class="form-control" name="roi_authorize_other_text">
      </div>

      <div class="mt-3">
        <label>Purpose</label>
        <input class="form-control" name="roi_purpose_text" placeholder="e.g., coordination with probation, employer verification, etc.">
      </div>
      <div class="mt-2">
        <label>Expiration (date/event/condition)</label>
        <input class="form-control" name="roi_expiration_text" placeholder="e.g., 1 year from date signed">
      </div>

      <div class="row mt-3">
        <div class="col-md-7">
          <label>Signature (type full legal name)</label>
          <input class="form-control" name="roi_signed_name">
        </div>
        <div class="col-md-5">
          <label>Signature Date</label>
          <input type="date" class="form-control" name="roi_signed_at">
        </div>
      </div>
    </div>
  ...
  <fieldset class="step">
    ...
    <div class="mb-3">
      ...
      <input type="text" id="counselor" name="counselor" class="form-control" value="Helen Bennett-Lopez">
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('input,select,textarea').forEach(el => {
    const key = 'intake_' + (el.id || el.name);
  ...
  // ---- Program-based show/hide (runs AFTER values are restored) ----
  const progSel = document.getElementById('program_select');
  const readmitBlock = document.getElementById('readmit_block');

  function setReadmitRequired(on) {
    const sel = document.getElementById('is_readmit');
    if (!sel) return;
    if (on) sel.setAttribute('required','required'); else sel.removeAttribute('required');
  }

  function syncProgramVisibility(code) {
    document.querySelectorAll('[data-show-programs]').forEach(el => {
      const list = (el.getAttribute('data-show-programs')||'').split(',').map(s=>s.trim());
      const show = code && list.includes(code);
      el.style.display = show ? '' : 'none';
      el.querySelectorAll('[data-required-when-visible="1"]').forEach(inp=>{
        if (show) inp.setAttribute('required','required'); else inp.removeAttribute('required');
      });
      if (!show) {
  ...
  function onProgramChange() {
    const code = progSel && progSel.value ? progSel.value : '';
    if (code) {
      readmitBlock.style.display = '';
      setReadmitRequired(true);
    } else {
      readmitBlock.style.display = 'none';
      setReadmitRequired(false);
    }
    syncProgramVisibility(code);
  }

  if (progSel) {
    progSel.addEventListener('change', onProgramChange);
    // ensure restored value (e.g., "bipp") applies immediately
    progSel.dispatchEvent(new Event('change'));
  }

  // keep program visibility correct when moving between steps
  ...
  function removeHiddenRequired() {
    // hidden steps
    document.querySelectorAll('.step:not(.active) [required]').forEach(el => el.removeAttribute('required'));
    // any containers hidden via .d-none
    document.querySelectorAll('.d-none [required]').forEach(el => el.removeAttribute('required'));
    // any containers hidden via inline display:none
    document.querySelectorAll('[style*="display: none"] [required]').forEach(el => el.removeAttribute('required'));
  }

  submitBtn.addEventListener('click', () => {
    removeHiddenRequired();
    // optional: also hide the red alert if it was showing
    const alertBox = document.getElementById('stepAlert');
  ...
  // ROI toggle show/hide
(function(){
  const roiEnable = document.getElementById('roi_enable');
  const roiBlock  = document.getElementById('roi_block');
  if (roiEnable && roiBlock) {
    const sync = () => { roiBlock.style.display = roiEnable.checked ? 'block' : 'none'; };
    roiEnable.addEventListener('change', sync);
    sync();
  }
})();
