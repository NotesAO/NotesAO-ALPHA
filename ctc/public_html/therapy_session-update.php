...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $date = trim($_POST["date"]);
		...
		$instructors_id = trim($_POST["instructors_id"]);
        if($instructors_id == '') $instructors_id = NULL;
    ...
    $stmt = $pdo->prepare("UPDATE therapy_session SET therapy_group_id=?,date=?,duration_minutes=?,curriculum_id=?,instructors_id=?,note=? WHERE id=?");

    if(!$stmt->execute([ $therapy_group_id,$date,$duration_minutes,$curriculum_id,$instructors_id,$note,$id  ])) {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$instructors_id = htmlspecialchars($row["instructors_id"]);
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
						...
						<div class="form-group">
                                <label>Instructor</label>
                                    <select class="form-control" id="instructors_id" name="instructors_id">
                                    <option value="">Not Specified</option>
                                    <?php
                                        $sql = "SELECT first_name, last_name, phone, id FROM instructors";
                                        $result = mysqli_query($link, $sql);
                                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            $duprow = $row;
                                            unset($duprow["id"]);
                                            $value = implode(" ", $duprow);
                                            if ($row["id"] == $instructors_id){
                                            echo '<option value="' . "$row[id]" . '"selected="selected">' . "$value" . '</option>';
                                            } else {
                                                echo '<option value="' . "$row[id]" . '">' . "$value" . '</option>';
                                        }
                                ...
                                <span class="form-text"><?php echo $instructors_id_err; ?></span>
                            </div>
						<div class="form-group">
                                <label>note</label>
