...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $vars = parse_columns('instructors', $_POST);
    $stmt = $pdo->prepare("UPDATE instructors SET first_name=?,last_name=?,email=?,phone=?,licensure=? WHERE id=?");

    if(!$stmt->execute([ $first_name,$last_name,$email,$phone,$licensure,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: instructors-read.php?id=$id");
    }
} else {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$licensure = htmlspecialchars($row["licensure"]);
...
<head>
    <meta charset="UTF-8">
    <title>NotesAO - Instructor Update</title>
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="instructors-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
