...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE absence SET date=?,excused=?,note=? WHERE id=?");

    if(!$stmt->execute([ $date,$excused,$note,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: absence-read.php?id=$id");
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$excused = htmlspecialchars($row["excused"]);
...
<head>
    <meta charset="UTF-8">
    <title>NotesAO - Absence Update</title>
