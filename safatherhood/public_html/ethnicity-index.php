...
<head>
    <meta charset="UTF-8">
    <title>NotesAO - Ethnicities</title>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">ethnicity Details</h2>
                        <a href="ethnicity-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="ethnicity-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="ethnicity-index.php" method="get">
                        <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM ethnicity";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('id', 'code', 'name');
                    $order = 'id';
                    ...
                    $sql = "SELECT * FROM ethnicity ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM ethnicity";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM ethnicity
                            WHERE CONCAT_WS (id,code,name)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                        ...
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . htmlspecialchars($row['id']) . "</td>";echo "<td>" . htmlspecialchars($row['code']) . "</td>";echo "<td>" . htmlspecialchars($row['name']) . "</td>";
                                        echo "<td>";
                                            echo "<a href='ethnicity-read.php?id=". $row['id'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='ethnicity-update.php?id=". $row['id'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='ethnicity-delete.php?id=". $row['id'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
