...
// Use $link if set elsewhere; otherwise fall back to $con
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'verify') {
        ...
        $stmt = mysqli_prepare($db, "UPDATE intake_packet SET staff_verified = 1, verified_by = ?, verified_at = NOW() WHERE intake_id = ?");
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'begin_merge') {
        ...
        $MERGE_FIELDS = [
          ...
          'phone_cell'        => ['label'=>'Phone (Cell)',        'intake_col'=>'phone_cell'],
          'email'             => ['label'=>'Email',               'intake_col'=>'email'],
          'emergency_name'    => ['label'=>'Emergency Name',      'intake_col'=>'emergency_name'],
          'emergency_relation'=> ['label'=>'Emergency Relation',  'intake_col'=>'emergency_relation'],
          ...
          'intake_date'       => ['label'=>'Intake Date',         'intake_col'=>'intake_date'],
        ];
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'apply_merge') {
        ...
        $MERGE_FIELDS = [
          'first_name','last_name','date_of_birth','gender_id','race_id','phone_cell','email',
          'emergency_name','emergency_relation','emergency_phone',
          'referral_type_id','cause_number','program_id','signature_date','intake_date',
        ];
        ...
        $st = mysqli_prepare($db,
            "UPDATE intake_packet SET imported_to_client=1, imported_client_id=? WHERE intake_id=?");
...
$referralLabel = $REFERRAL_MAP[(int)($row['referral_type_id'] ?? -1)] ?? ($row['referral_type_id'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'import_victim') {
    ...
    if ($st = mysqli_prepare($db, $sql)) {
        // match 'isssiiisssssss' from client-victim-create.php
        ...
        <div class="row">
          <div class="col-md-4">
            ...
            <?php
              ...
              $gender_map = [1 => 'Other', 2 => 'Male', 3 => 'Female'];
              $gender_val = (int)($row['gender_id'] ?? 0);
              $gender_label = $gender_map[$gender_val] ?? ($row['gender_id'] ?? '');
            ?>
