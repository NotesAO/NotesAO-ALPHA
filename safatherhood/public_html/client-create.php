...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if(!$hasErrors){
      ...
      "INSERT INTO client (
        ...
        emergency_contact,orientation_date,exit_date,exit_reason_id,exit_note,
      ...
      // Seed ledger with fixed $25 orientation charge and payment (same as import)
      $created_client_id = $pdo->lastInsertId();
      ...
      // 1) Charge: -25.00
      $lg->execute([$created_client_id, -25.00, 'intake/orientation fee']);

      // 2) Payment: +25.00
      $lg->execute([$created_client_id,  25.00, 'intake/orientation payment']);

      // (no need to close PDO statements; they auto-close)
...
<div class="row">
  ...
  <div class="col-12 col-sm-6 col-md-4 col-lg-2">
    <div class="form-group">
      ...
      <?php
        // Normalize current value and clamp to allowed options.
        $fee_val = isset($fee) ? (string)(int)$fee : '';
        if (!in_array($fee_val, ['10','15','25'], true)) { $fee_val = ''; }
      ?>
      <select id="fee" name="fee" class="form-control <?= !empty($fee_err) ? 'is-invalid' : '' ?>" required>
        <option value="" disabled <?= $fee_val === '' ? 'selected' : '' ?>>Select fee</option>
        <option value="10" <?= $fee_val === '10' ? 'selected' : '' ?>>$10</option>
        <option value="15" <?= $fee_val === '15' ? 'selected' : '' ?>>$15</option>
        <option value="25" <?= $fee_val === '25' ? 'selected' : '' ?>>$25</option>
      </select>
...
function validate(el){
  ...
  switch(el.id){
    ...
    case F.reqSess:
      state=['20','8'].includes(v)?'valid':(/^\d+$/.test(v)&&+v>0?'warning':'invalid');
      break;
    case F.fee:{
      let f=parseFloat(v);
      if([32,30].includes(f))f=30; else if([21,20].includes(f))f=20;
      else if([16,15].includes(f))f=15;
      state=[15,20,30,25].includes(f)?'valid':(f>0&&f<=100?'warning':'invalid');
