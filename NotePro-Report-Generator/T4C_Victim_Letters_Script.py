...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for T4C clients.

Usage (CLI)
------------
python T4C_Victim_Letters_Script.py \
...
Required Word templates (names are fixed):
  Template.T4C Victim Entrance.docx
  Template.T4C Victim Completion.docx
  Template.T4C Victim Exit.docx
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate T4C Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"T4C_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("T4C Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.T4C Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.T4C Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.T4C Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to T4C programs
PROGRAM_FILTER = ["Thinking for a Change"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No T4C records found – exiting.")
    sys.exit(0)
