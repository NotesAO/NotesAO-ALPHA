...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for DWI Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWI_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWI Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = need_template('Template.DWI Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to DWI + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be DWI (male or female). Adjust if your CSV uses different strings.
dwi_filter = df['program_name'].isin(['DWI'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[dwi_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching DWI + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("DWI Behavior Contract Script completed successfully.")
