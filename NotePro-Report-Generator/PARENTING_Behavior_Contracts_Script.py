...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for Parenting Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'PARENTING_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("PARENTING Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.PARENTING Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to PARENTING + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be PARENTING (male or female). Adjust if your CSV uses different strings.
parenting_filter = df['program_name'].isin(['Parenting Education', 'Parenting'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[parenting_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching PARENTING + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("PARENTING Behavior Contract Script completed successfully.")
