#!/usr/bin/env python3
# T4C_Exit_Notices_and_Exit_Progress_Reports.py
...
# 1) Parse arguments
##############################################################################
parser = argparse.ArgumentParser(description='Generate T4C Exit Notices + Exit Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'T4C_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("T4C Exit Notices + Exit Progress Reports Script started.")
...
# 5) Template paths
##############################################################################
exit_notice_template_path = os.path.join(templates_dir, 'Template.T4C Exit Notice.docx')
exit_progress_report_template_path = os.path.join(templates_dir, 'Template.T4C Exit Progress Report.docx')
...
def parse_multiple_formats(value, formats=None):
    ...
    if formats is None:
        # Includes expansions for T4C Progress logic
        formats = [
            '%m/%d/%Y', '%Y-%m-%d', '%m/%d/%y',
...
def transform_calendar_cell(cell_value):
    """
    Preserve multiple marks in a single calendar day for Exit Notices:
      P -> ✅ (Present)
      X -> ❌ (Unexcused)
      E -> ✖️ (Excused)

    Handles exact mojibake patterns from report5 dumps, including cases where
    the second mark is truncated to a lone 'â'. Returns: "<day> <marks>".
    """
    ...
    import re
    # 0) Letters (if present) -> canonical symbols
    ...
    #    Orphan-tail patterns: first symbol complete, second is just 'â'
    # ------------------------------------------------------------------

    # Present + orphan → Present + Unexcused (per your data)
    ...
    # Double Present (incl. compact/split variants)
    ...
    # Unexcused + Excused (remaining explicit forms)
    ...
    # -------------------------------------------------------------
    # 3) Cleanup: remove orphan mojibake only when next to a symbol
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 4) Extract first day number + symbols in encounter order
    # -------------------------------------------------------------
    m = re.search(r'\b(\d{1,2})\b', raw)
    day = m.group(1) if m else None
    marks = ''.join(re.findall(r'✅|❌|✖️', raw))

    # Exit templates expect a space between day and marks
    out = (f"{day} {marks}" if day else marks).strip()

    # Final tiny cleanups (display artifacts)
    out = out.replace("Â", "").replace("â", "").replace("œ", "").strip()
    return out


##############################################################################
# Fill placeholders with the same logic from T4C Progress script
##############################################################################
def fill_placeholders(row):
    """
    This function merges the standard T4C progress placeholders logic
...
# Must be T4C. Must have exit_date in the date range. Must have exit_reason not in ["Not Exited", "Completion of Program"].
df['exit_date'] = pd.to_datetime(df['exit_date'], errors='coerce')
df = df.dropna(subset=['exit_date'])  # drop if exit_date can't parse

df = df[df['program_name'] == 'Thinking for a Change']
...
def generate_documents_for_row(row):
    ...
    # 1) Render T4C Exit Notice
    exit_pdf = render_and_save(exit_notice_template_path, "T4C Exit Notice", context)

    # 2) Render T4C Exit Progress Report
    progress_pdf = render_and_save(exit_progress_report_template_path, "T4C Exit Progress Report", context)
...
logging.info("T4C Exit Notices and Exit Progress Reports have been generated and organized.")
print("T4C Exit Notices and Exit Progress Reports have been generated and organized.")
