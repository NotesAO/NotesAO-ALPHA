...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for SAE clients.

Usage (CLI)
------------
python SAE_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate SAE Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"SAE_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("SAE Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.SAE Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.SAE Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.SAE Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to SAE programs
PROGRAM_FILTER = ["SAE"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No SAE records found – exiting.")
    sys.exit(0)
