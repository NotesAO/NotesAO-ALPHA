...
parser = argparse.ArgumentParser(description='Generate DWIE Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWIE_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWIE Progress Reports Script started.")
...
progress_report_template_path         = need_template('Template.DWIE Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.DWIE Progress Report Virtual.docx')
...
# Filter for DWIE (male/female) and Not Exited clients
...
if df.empty:
    logging.warning("No records found for DWIE Not Exited clients. No documents will be generated.")
    sys.exit(0)
...
def render_and_save(template_path, doc_type, context, output_dir):
    try:
        ...
        docx_filename = f"{last_name} {first_name} DWIE PReport.docx"
...
def generate_documents(row, output_dir_path):
    ...
    report_template = virtual_progress_report_template_path if ("Virtual" in group_name or "Women" in group_name) else progress_report_template_path
    if not report_template:
        record_error("[FATAL] Cannot render DWIE Progress Report – template missing.")
        return  # or continue
...
logging.info("DWIE Progress Reports Script completed successfully.")
