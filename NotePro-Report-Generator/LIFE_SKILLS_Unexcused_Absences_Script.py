...
parser = argparse.ArgumentParser(description='Generate Life Skills Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'LIFE_SKILLS_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Life Skills Unexcused Absences Script started.")
...
# Template path
template_file_path = os.path.join(templates_dir, 'Template.LIFE SKILLS Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} Life Skills Unexcused Absence.docx"
...
logging.info("Life Skills Unexcused Absences Script completed successfully.")
