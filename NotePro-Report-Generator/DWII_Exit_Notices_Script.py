...
parser = argparse.ArgumentParser(description='Generate DWII Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWII_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWII Exit Notices Script started.")

base_dir = os.path.dirname(os.path.abspath(__file__))
templates_dir = args.templates_dir

# --- env overrides + templates root ---
program_code  = os.environ.get('NP_PROGRAM_CODE', 'DWII')
...
exit_notice_template_path             = need_template('Template.DWII Exit Notice.docx')
progress_report_template_path         = need_template('Template.DWII Exit Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.DWII Exit Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for DWII 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for DWII 27-week: attended={attended}")
            return 'Alert'
...
df = df[df['program_name'].isin(['DWII'])]
...
def generate_documents(row, output_dir_path):
    ...
    report_template = (
        virtual_progress_report_template_path
        if ("Virtual" in str(group_name) or "Women" in str(group_name))
        else progress_report_template_path
    )
    ...
    # Exit Notice
    if exit_notice_template_path:
        exit_pdf = render_and_save(
            exit_notice_template_path, "Exit Notice", context, generated_documents_dir
        )
    else:
        record_error("[FATAL] Cannot render DWII Exit Notice – template missing.")

    # Exit Progress Report (virtual vs. in-person already handled by report_template)
    if report_template:
        progress_pdf = render_and_save(
            report_template, report_suffix, context, generated_documents_dir
        )
    else:
        record_error("[FATAL] Cannot render DWII Exit Progress Report – template missing.")
...
logging.info("DWII Exit Notices Script completed successfully.")
