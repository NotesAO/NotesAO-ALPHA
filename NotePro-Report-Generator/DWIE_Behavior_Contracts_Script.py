...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for DWIE Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWIE_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWIE Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = need_template('Template.DWIE Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to DWIE + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be DWIE (male or female). Adjust if your CSV uses different strings.
dwie_filter = df['program_name'].isin(['DWIE'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[dwie_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching DWIE + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("DWIE Behavior Contract Script completed successfully.")
