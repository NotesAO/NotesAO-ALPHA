...
parser = argparse.ArgumentParser(description='Generate DOEP Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DOEP_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DOEP Exit Notices Script started.")
...
exit_notice_template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Exit Notice.docx')
if not exit_notice_template_path:
    record_error(f"[FATAL] Missing template Template.DOEP Exit Notice.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")

progress_report_template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Exit Progress Report.docx')
if not progress_report_template_path:
    record_error(f"[FATAL] Missing template Template.DOEP Exit Progress Report.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")

virtual_progress_report_template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Exit Progress Report Virtual.docx')
if not virtual_progress_report_template_path:
    record_error(f"[FATAL] Missing template Template.DOEP Exit Progress Report Virtual.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for DOEP 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for DOEP 27-week: attended={attended}")
            return 'Alert'
...
def generate_documents(row, output_dir_path):
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "DOEP Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"DOEP {report_suffix}", context, output_dir_path)
...
logging.info("DOEP Exit Notices Script completed successfully.")
