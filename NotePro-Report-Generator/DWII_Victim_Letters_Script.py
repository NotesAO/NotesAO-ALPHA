...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for DWII clients.

Usage (CLI)
------------
python DWII_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate DWII Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"DWII_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("DWII Victim Letters Script started.")
...
LETTER_TYPES = {
    "entrance": {
        "template": "Template.DWII Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.DWII Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.DWII Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to DWII programs
PROGRAM_FILTER = ["DWII"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No DWII records found – exiting.")
    sys.exit(0)
