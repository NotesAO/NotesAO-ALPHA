...
# -------- Template resolution with clinic/default fallbacks --------
template_path = None
for name in ('Template.DWII Enrollment Letter.docx', 'Template.Enrollment Letter.docx', 'Template.DWII Enroll.docx', 'Template.DWII Entrance Notification.docx'):
    p = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, name)
    if p:
        template_path = p
        break
if not template_path:
    # soft-fail; keep processing rows but nothing will render
    record_error(f"[FATAL] No DWII enrollment template found under clinic/default fallbacks in {_TEMPLATES_ROOT}. Tried DWII Enrollment Letter / Enrollment Letter / DWII Enroll / DWII Entrance Notification")
