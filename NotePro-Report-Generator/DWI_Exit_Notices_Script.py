...
parser = argparse.ArgumentParser(description='Generate DWI Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWI_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWI Exit Notices Script started.")
...
exit_notice_template_path             = need_template('Template.DWI Exit Notice.docx')
progress_report_template_path         = need_template('Template.DWI Exit Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.DWI Exit Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for DWI 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for DWI 27-week: attended={attended}")
            return 'Alert'
...
def generate_documents(row, output_dir_path):
    ...
    if exit_notice_template_path:
        exit_pdf = render_and_save(exit_notice_template_path, "DWI Exit Notice", context, output_dir_path)
    else:
        exit_pdf = None

    group_name = context.get("group_name", "")
    report_template = (virtual_progress_report_template_path if ("Virtual" in group_name or "Women" in group_name)
                    else progress_report_template_path)
    if report_template:
        progress_pdf = render_and_save(report_template, "DWI Exit PReport", context, output_dir_path)
    else:
        progress_pdf = None
...
logging.info("DWI Exit Notices Script completed successfully.")
