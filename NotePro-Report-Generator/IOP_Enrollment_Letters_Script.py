...
# --- choose enrollment letter template from clinic/default/BIPP fallbacks ---
CANDIDATE_TEMPLATES = [
    'Template.Enrollment Letter.docx',
    'Template.IOP Enrollment Letter.docx',
    'Template.IOP Enroll.docx',
    'Template.IOP Entrance Notification.docx',  # last-resort fallback
]

enrollment_letter_template_path = None
for name in CANDIDATE_TEMPLATES:
    p = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, name)
    if p:
        enrollment_letter_template_path = p
        break

if not enrollment_letter_template_path:
    # log it, but do not hard-stop the run
    record_error(
        f"[FATAL] Missing enrollment letter template. Tried: {', '.join(CANDIDATE_TEMPLATES)} "
        f"under clinic/default fallbacks in {_TEMPLATES_ROOT}."
    )
    # keep going; per-row logic should skip gracefully if this stays None
...
for idx, row in df_filtered.iterrows():
    try:
        if not enrollment_letter_template_path:
            record_error("Cannot render IOP Enrollment Letter – template missing.")
            return
