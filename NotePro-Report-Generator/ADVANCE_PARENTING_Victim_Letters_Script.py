"""
ADV_PARENTING_Victim_Letters_Script.py
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for ADV PARENTING clients.

Usage (CLI)
------------
python ADV PARENTING_Victim_Letters_Script.py \
...
Required Word templates (names are fixed):
  Template.ADV PARENTING Victim Entrance.docx
  Template.ADV PARENTING Victim Completion.docx
  Template.ADV PARENTING Victim Exit.docx
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate ADV PARENTING Victim Notification Letters")
...
# env overrides (match BIPP/AEMP)
PROGRAM_CODE  = os.environ.get('NP_PROGRAM_CODE', 'ADV PARENTING')
...
LOG_PATH = os.path.join(LOG_DIR, f"ADVANCE_Parenting_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("ADV PARENTING Victim Letters Script started.")

errors_txt = os.path.join(GENERATED_DIR, 'errors.txt')
...
LETTER_TYPES = {
    "entrance": {
        "template": "Template.ADV PARENTING Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.ADV PARENTING Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.ADV PARENTING Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to ADV PARENTING programs
PROGRAM_FILTER = ["ADVANCE Parenting"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No ADV PARENTING records found – exiting.")
    sys.exit(0)
...
for idx, row in df.iterrows():
    ...
    if not template_file:
        record_error(f"[FATAL] Missing template {tmpl_name} under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
        continue  # skip this row; do not crash run
