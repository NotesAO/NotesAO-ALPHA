...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for IOP Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'IOP_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("IOP Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = need_template('Template.IOP Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to IOP + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be IOP (male or female). Adjust if your CSV uses different strings.
iop_filter = df['program_name'].isin(['IOP'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[iop_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching IOP + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
def render_and_save_behavior_contract(row_data, output_dir_path):
    if not behavior_contract_template_path:
        record_error("Cannot render IOP Behavior Contract – template missing.")
        return
...
logging.info("IOP Behavior Contract Script completed successfully.")
