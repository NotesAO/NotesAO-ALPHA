...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for ADV PARENTING Clients')
...
# --- env overrides (match BIPP/AEMP pattern) ---
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'ADV_PARENTING_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("ADV Parenting Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = need_template('Template.ADV PARENTING Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to ADV PARENTING + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be ADV PARENTING (male or female). Adjust if your CSV uses different strings.
adv_parenting_filter = df['program_name'].isin(['ADVANCE Parenting'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[adv_parenting_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching ADV PARENTING + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("ADV PARENTING Behavior Contract Script completed successfully.")
