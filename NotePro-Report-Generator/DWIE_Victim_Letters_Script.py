...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for DWIE clients.

Usage (CLI)
------------
python DWIE_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate DWIE Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"DWIE_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("DWIE Victim Letters Script started.")
...
LETTER_TYPES = {
    "entrance": {
        "template": "Template.DWIE Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.DWIE Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.DWIE Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to DWIE programs
PROGRAM_FILTER = ["DWIE"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No DWIE records found – exiting.")
    sys.exit(0)
