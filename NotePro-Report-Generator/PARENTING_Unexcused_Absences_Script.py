...
parser = argparse.ArgumentParser(description='Generate PARENTING Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'PARENTING_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("PARENTING Unexcused Absences Script started.")
...
# Template path
template_file_path = os.path.join(templates_dir, 'Template.PARENTING Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} PARENTING Unexcused Absence.docx"
...
logging.info("PARENTING Unexcused Absences Script completed successfully.")
