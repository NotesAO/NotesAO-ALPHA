...
parser = argparse.ArgumentParser(description='Generate IOP Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'IOP_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("IOP Exit Notices Script started.")

base_dir = os.path.dirname(os.path.abspath(__file__))
templates_dir = args.templates_dir

# --- env overrides + templates root ---
program_code  = os.environ.get('NP_PROGRAM_CODE', 'IOP')
...
def need_template(filename: str):
    ...
    return p  # may be None; callers should skip gracefully if None


exit_notice_template_path             = need_template('Template.IOP Exit Notice.docx')
progress_report_template_path         = need_template('Template.IOP Exit Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.IOP Exit Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for IOP 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for IOP 27-week: attended={attended}")
            return 'Alert'
...
df = df[df['program_name'].isin(['IOP'])]
...
def generate_documents(row, output_dir_path):
    # skip gracefully if any required template is missing
    if not exit_notice_template_path:
        record_error("Cannot render Exit Notice – template missing.")
        return
    if "Virtual" in str(row.get('group_name','')) or "Women" in str(row.get('group_name','')):
        if not virtual_progress_report_template_path:
            record_error("Cannot render Virtual Exit Progress Report – template missing.")
            return
    else:
        if not progress_report_template_path:
            record_error("Cannot render Exit Progress Report – template missing.")
            return
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "IOP Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"IOP {report_suffix}", context, output_dir_path)
...
logging.info("IOP Exit Notices Script completed successfully.")
