...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for Life Skills Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'LIFE_SKILLS_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Life Skills Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.LIFE SKILLS Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to Life Skills + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be Life Skills (male or female). Adjust if your CSV uses different strings.
life_skills_filter = df['program_name'].isin(['Life Skills'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[life_skills_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching Life Skills + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("Life Skills Behavior Contract Script completed successfully.")
