...
parser = argparse.ArgumentParser(description='Generate DWII Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWII_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWII Unexcused Absences Script started.")
...
# Template path
template_file_path = need_template('Template.DWII Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} DWII Unexcused Absence.docx"
...
logging.info("DWII Unexcused Absences Script completed successfully.")
