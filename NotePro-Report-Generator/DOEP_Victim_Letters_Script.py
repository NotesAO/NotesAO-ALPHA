...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for DOEP clients.

Usage (CLI)
------------
python DOEP_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate DOEP Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"DOEP_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("DOEP Victim Letters Script started.")
...
LETTER_TYPES = {
    "entrance": {
        "template": "Template.DOEP Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.DOEP Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.DOEP Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to DOEP programs
PROGRAM_FILTER = ["DOEP"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No DOEP records found – exiting.")
    sys.exit(0)
...
for idx, row in df.iterrows():
    ...
    # Template path
    template_name = LETTER_TYPES[applicable_type]["template"]
    template_file = resolve_template(_TEMPLATES_ROOT, CLINIC, program_code, template_name)
