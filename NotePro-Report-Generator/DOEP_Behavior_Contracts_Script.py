...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for DOEP Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DOEP_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DOEP Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = resolve_template(
    _TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Behavior Contract.docx'
)
if not behavior_contract_template_path:
    record_error(f"[FATAL] Missing template Template.DOEP Behavior Contract.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
...
# ------------------------------------------------------------------------------
# 7) Filter to DOEP + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be DOEP (male or female). Adjust if your CSV uses different strings.
doep_filter = df['program_name'].isin(['DOEP'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[doep_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching DOEP + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("DOEP Behavior Contract Script completed successfully.")
