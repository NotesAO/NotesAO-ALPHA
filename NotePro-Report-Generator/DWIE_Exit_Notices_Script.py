...
parser = argparse.ArgumentParser(description='Generate DWIE Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWIE_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWIE Exit Notices Script started.")
...
exit_notice_template_path             = need_template('Template.DWIE Exit Notice.docx')
progress_report_template_path         = need_template('Template.DWIE Exit Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.DWIE Exit Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for DWIE 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for DWIE 27-week: attended={attended}")
            return 'Alert'
...
def render_and_save(template_path, doc_type, context, output_dir):
    try:
        ...
        if not exit_notice_template_path:
            record_error("[FATAL] Cannot render Exit Notice – template missing.")
            # skip creating Exit Notice

        report_tmpl = virtual_progress_report_template_path if use_virtual else progress_report_template_path
        if not report_tmpl:
            record_error("[FATAL] Cannot render Exit Progress Report – template missing.")
            # skip creating Progress Report
...
def generate_documents(row, output_dir_path):
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "DWIE Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"DWIE {report_suffix}", context, output_dir_path)
...
logging.info("DWIE Exit Notices Script completed successfully.")
