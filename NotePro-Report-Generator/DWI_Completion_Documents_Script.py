...
parser = argparse.ArgumentParser(description='Generate DWI Completion Documents')
...
program_code  = os.environ.get('NP_PROGRAM_CODE', 'DWI')
...
completion_letter_file_path             = need_template('Template.DWI Completion Letter.docx')
completion_certificate_file_path        = need_template('Template.DWI Completion Certificate.docx')
completion_report_file_path             = need_template('Template.DWI Completion Progress Report.docx')
completion_virtual_report_file_path     = need_template('Template.DWI Completion Progress Report Virtual.docx')
...
df = df[df['program_name'].isin(['DWI'])]
...
if df_filtered.empty:
    ...
    for index, row in df_filtered.iterrows():
        ...
        try:
            ...
            # inside the for index, row in df_filtered.iterrows(): loop, after context is built

            # Letter
            if completion_letter_file_path:
                letter_docx = render_and_save(completion_letter_file_path, "DWI Comp Letter", context, generated_documents_dir)

            # Certificate
            if completion_certificate_file_path:
                cert_docx = render_and_save(completion_certificate_file_path, "DWI Comp Cert", context, generated_documents_dir)

            # Progress Report (virtual vs in-person)
            progress_template = (
                completion_virtual_report_file_path
                if "Virtual" in str(context.get('group_name',''))
                else completion_report_file_path
            )
            if progress_template:
                progress_docx = render_and_save(progress_template, "DWI Comp PReport", context, generated_documents_dir)

            # (then append any non-None docx paths to your list for PDF conversion)
            for p in (letter_docx, cert_docx, progress_docx):
                if p:
                    generated_docx_files.append(p)
