#!/usr/bin/env python3
# MRT_Exit_Notices_and_Exit_Progress_Reports.py
...
# 1) Parse arguments
##############################################################################
parser = argparse.ArgumentParser(description='Generate MRT Exit Notices + Exit Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MRT_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("MRT Exit Notices + Exit Progress Reports Script started.")
...
# 5) Template paths
##############################################################################
exit_notice_template_path = os.path.join(templates_dir, 'Template.MRT Exit Notice.docx')
exit_progress_report_template_path = os.path.join(templates_dir, 'Template.MRT Exit Progress Report.docx')
...
def format_balance(balance):
    # Normalize, parse, and always format as currency (including zero)
    try:
        s = str(balance).strip()
        s = s.replace('$','').replace(',','')
        v = float(s if s not in ('', 'nan', 'NaN', None) else 0)
        return f"${v:,.2f}"
    except Exception:
        return "$0.00"
...
def parse_multiple_formats(value, formats=None):
    ...
    if formats is None:
        # Includes expansions for MRT Progress logic
        formats = [
            '%m/%d/%Y', '%Y-%m-%d', '%m/%d/%y',
...
# --- Progress-style checkbox + mid-derived logic ----------------------------
def _chk(ok):
    return '\u2714' if ok else '\u2716'
...
def _mrt_mid_derived(row):
    ...
    required   = _f(row.get('required_sessions', 0))
    unexcused  = _f(row.get('absence_unexcused', 0))
    ...
    ratio_ok = ((required - unexcused) / required >= 0.90) if required > 0 else False
    row['consistent_attendance'] = _chk(ratio_ok)

    # three_or_more_absences text
    try:
    ...
    # group rules / agreement / dialogue / accountability
    def _is_yes(v):
...
##############################################################################
# Fill placeholders with the same logic from MRT Progress script
##############################################################################
def fill_placeholders(row):
    """
    This function merges the standard MRT progress placeholders logic
...
# Must be MRT. Must have exit_date in the date range. Must have exit_reason not in ["Not Exited", "Completion of Program"].
df['exit_date'] = pd.to_datetime(df['exit_date'], errors='coerce')
df = df.dropna(subset=['exit_date'])  # drop if exit_date can't parse

df = df[df['program_name'] == 'MRT']
...
# stage-of-change and fees AFTER placeholders
...
def _fee_flag(v):
    try:
        return "No" if float(v or 0) == 0 else "Yes"
    except Exception:
        return "Yes"

df['feeproblems'] = df['balance'].apply(_fee_flag)
df['balance'] = df['balance'].apply(format_balance)


# Progress-style mid-field derivations for exit docs
...
def generate_documents_for_row(row):
    ...
    # 1) Render MRT Exit Notice
    exit_pdf = render_and_save(exit_notice_template_path, "MRT Exit Notice", context)

    # 2) Render MRT Exit Progress Report
    progress_pdf = render_and_save(exit_progress_report_template_path, "MRT Exit Progress Report", context)
...
logging.info("MRT Exit Notices and Exit Progress Reports have been generated and organized.")
print("MRT Exit Notices and Exit Progress Reports have been generated and organized.")
