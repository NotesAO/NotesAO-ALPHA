...
CANDIDATE_TEMPLATES = [
    ...
    'Template.ADV PARENTING Enrollment Letter.docx',
    'Template.ADV PARENTING Enroll.docx',
    'Template.ADV PARENTING Entrance Notification.docx',  # last-resort fallback
]
template_path = None
for name in CANDIDATE_TEMPLATES:
    cand = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, name)
    if cand:
        template_path = cand
        break
if not template_path:
    record_error(f"[FATAL] No enrollment DOCX template found. Tried: {CANDIDATE_TEMPLATES} under {_TEMPLATES_ROOT}")
    sys.exit(0)
