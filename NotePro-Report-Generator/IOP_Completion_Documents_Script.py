...
parser = argparse.ArgumentParser(description='Generate IOP Completion Documents')
...
def need_template(filename: str):
    p = resolve_template(_TEMPLATES_ROOT, CLINIC, program_code, filename)
    if not p:
...
completion_letter_file_path         = need_template('Template.IOP Completion Letter.docx')
completion_certificate_file_path    = need_template('Template.IOP Completion Certificate.docx')
completion_report_file_path         = need_template('Template.IOP Completion Progress Report.docx')
completion_virtual_report_file_path = need_template('Template.IOP Completion Progress Report Virtual.docx')
...
def render_and_save(template_path, doc_type, context, output_dir):
    if not template_path:
        record_error(f"Missing template for {doc_type}; skipping.")
        return None
    try:
...
if df_filtered.empty:
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            letter_docx = render_and_save(completion_letter_file_path, "IOP Comp Letter", context, generated_documents_dir)
            certificate_docx = render_and_save(completion_certificate_file_path, "IOP Comp Cert", context, generated_documents_dir)
            progress_template = (
                completion_virtual_report_file_path
                if "Virtual" in str(context.get('group_name','')) or "Women" in str(context.get('group_name',''))
                else completion_report_file_path
            )
            if progress_template:
                progress_docx = render_and_save(progress_template, "IOP Comp PReport", context, generated_documents_dir)
            else:
                record_error("Missing IOP Completion Progress Report template; skipping report.")

            progress_report_docx = render_and_save(progress_template, "IOP PReport", context, generated_documents_dir)
