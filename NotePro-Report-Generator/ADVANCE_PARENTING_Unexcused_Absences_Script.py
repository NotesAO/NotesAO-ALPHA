...
parser = argparse.ArgumentParser(description='Generate ADV PARENTING Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'ADVANCE_Parenting_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("ADV PARENTING Unexcused Absences Script started.")
...
# Template path
template_file_path = need_template('Template.ADV PARENTING Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} ADV PARENTING Unexcused Absence.docx"
...
logging.info("ADV PARENTING Unexcused Absences Script completed successfully.")
