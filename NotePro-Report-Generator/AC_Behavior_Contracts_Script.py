...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for AC Clients')
...
# --- env overrides to support clinic/default fallbacks ---
...
# templates root from either ".../templates" or ".../templates/<clinic>"
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'AC_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("AC Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.AC Behavior Contract.docx')
if not behavior_contract_template_path:
    record_error(f"[FATAL] Missing template Template.AC Behavior Contract.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
...
# ------------------------------------------------------------------------------
# 7) Filter to AC + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be AC (male or female). Adjust if your CSV uses different strings.
ac_filter = df['program_name'].isin(['Anger Control', 'Anger (male)', 'Anger (female)', 'Anger Management'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[ac_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching AC + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
def render_and_save_behavior_contract(row_data, output_dir_path):
    if not behavior_contract_template_path:
        return None  # soft-skip the whole run if template is missing
...
logging.info("AC Behavior Contract Script completed successfully.")
