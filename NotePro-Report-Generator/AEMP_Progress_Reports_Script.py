...
parser = argparse.ArgumentParser(description='Generate AEMP Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'AEMP_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("AEMP Progress Reports Script started.")
...
# Compute templates root after env overrides
...
progress_report_template_path         = need_template('Template.AEMP Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.AEMP Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for AEMP 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for AEMP 27-week: attended={attended}")
            return 'Alert'
...
# Filter for AEMP (male/female) and Not Exited clients
df['exit_date'] = pd.to_datetime(df['exit_date'], errors='coerce')

df = df[df['program_name'].isin(['AEMP'])]
df = df[df['exit_reason'] == 'Not Exited']

if df.empty:
    logging.warning("No records found for AEMP Not Exited clients. No documents will be generated.")
    sys.exit(0)
...
def render_and_save(template_path, doc_type, context, output_dir):
    try:
        ...
        docx_filename = f"{last_name} {first_name} AEMP PReport.docx"
...
logging.info("AEMP Progress Reports Script completed successfully.")
