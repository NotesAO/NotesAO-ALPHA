...
parser = argparse.ArgumentParser(description='Generate DOEP Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DOEP_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DOEP Progress Reports Script started.")
...
progress_report_template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Progress Report.docx')
if not progress_report_template_path:
    record_error(f"[FATAL] Missing template Template.DOEP Progress Report.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")

virtual_progress_report_template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Progress Report Virtual.docx')
if not virtual_progress_report_template_path:
    record_error(f"[FATAL] Missing template Template.DOEP Progress Report Virtual.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
...
# Filter for DOEP (male/female) and Not Exited clients
...
if df.empty:
    logging.warning("No records found for DOEP Not Exited clients. No documents will be generated.")
    sys.exit(0)
...
def render_and_save(template_path, doc_type, context, output_dir):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} DOEP Stage PReport.docx"
...
def generate_documents(row, output_dir_path):
    ...
    # NEW: define group_name before using it
    ...
    if not report_template:
        record_error("[FATAL] No progress report template available (both virtual and standard missing).")
        return
...
logging.info("DOEP Progress Reports Script completed successfully.")
