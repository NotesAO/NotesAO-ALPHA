...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for Marijuana Education Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MARI_EDU_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("MARI EDU Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.MARIJUANA EDU Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to Marijuana Education + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be Marijuana Education (male or female). Adjust if your CSV uses different strings.
mari_edu_filter = df['program_name'].isin(['Marijuana Education'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[mari_edu_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching Marijuana Education + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("Marijuana Education Behavior Contract Script completed successfully.")
