...
# 1) Parse arguments
##############################################################################
parser = argparse.ArgumentParser(description='Generate TIPS Completion Documents (Letter, Certificate, Completion Progress Report)')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'TIPS_Completion_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("TIPS Completion Documents Script started.")
...
# 5) Template paths
##############################################################################
completion_letter_template = os.path.join(templates_dir, 'Template.TIPS Completion Letter.docx')
completion_certificate_template = os.path.join(templates_dir, 'Template.TIPS Completion Certificate.docx')
completion_report_template = os.path.join(templates_dir, 'Template.TIPS Completion Progress Report.docx')
...
def parse_multiple_formats(value, formats=None):
    """Try multiple date formats for a single string value (expanded for TIPS logic)."""
    if formats is None:
        formats = [
            '%m/%d/%Y', '%Y-%m-%d', '%m/%d/%y',
            '%b-%Y', '%B %Y',
...
##############################################################################
# fill_placeholders for TIPS completion progress logic
##############################################################################
def fill_placeholders(row):
    """
    Merges the standard TIPS progress placeholders logic (like attendance squares,
...
# Filter only TIPS, exit_reason == 'Completion of Program', exit_date in range
df['exit_date'] = pd.to_datetime(df['exit_date'], errors='coerce')
df = df.dropna(subset=['exit_date'])

df = df[
    (df['program_name'] == 'TIPS (Theft)') &
...
def generate_documents_for_row(row):
    ...
    # 1) TIPS Completion Progress Report
    pdf_report = render_and_save(completion_report_template, "TIPS Completion Progress Report", context)

    # 2) TIPS Completion Letter
    pdf_letter = render_and_save(completion_letter_template, "TIPS Completion Letter", context)

    # 3) TIPS Completion Certificate
    pdf_certificate = render_and_save(completion_certificate_template, "TIPS Completion Certificate", context)
...
logging.info("TIPS Completion Documents have been generated and organized.")
print("TIPS Completion Documents have been generated and organized.")
