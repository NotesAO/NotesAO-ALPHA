...
parser = argparse.ArgumentParser(description='Generate MRT Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MRT_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("MRT Progress Reports Script started.")
...
template_file_path = os.path.join(templates_dir, 'Template.MRT Progress Report.docx')
...
def format_balance(balance):
    try:
        return f"${int(float(balance or 0))}"
    except Exception:
        return "$0"
...
ABSENCE_LIMIT = 3   # “3+ absences” threshold
...
def _mrt_mid_derived(row):
    ...
    row['attended_initial_assessessment'] = row['attended_initial_assessment']  # handle template typo

    # consistent attendance = within 10% unexcused (same rule you used on BIPP)
    if required > 0:
    ...
    # show “3+ absences” as ✓ / ✖ too
    # Three or more absences text (BIPP-style)
    absences = int(unexcused)  # unexcused already parsed above
    if absences >= 3:
    ...
    row['participated_in_dialoge']          = row['participated_in_dialogue']  # handle template typo
    row['accountability_consistent']        = _chk(takes_resp)
    row['participated_in_dialog']           = row['participated_in_dialogue']  # handle template typo
    return row
...
# Filter for MRT Not Exited clients
...
df = ensure_columns(df, [
    'required_sessions','absence_unexcused','orientation_date',
...
def generate_documents_for_row(row):
    ...
    pdf_path = render_and_save(template_file_path, "MRT Progress Report", context)
...
logging.info("MRT Progress Reports have been generated and organized.")
print("MRT Progress Reports have been generated and organized.")
...
# make sure needed columns exist (add balance here too)
df = ensure_columns(df, [
...
# derive all mid-section checkmarks (attendance, parameters, accountability)
df = df.apply(_mrt_mid_derived, axis=1)

# your existing placeholder formatting
df = df.apply(fill_placeholders, axis=1)
df.fillna('', inplace=True)
df.replace(['nan', 'NaN'], '', inplace=True)

# stage-of-change and fees AFTER placeholders/derivations
