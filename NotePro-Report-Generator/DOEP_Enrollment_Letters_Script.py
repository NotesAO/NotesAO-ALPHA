...
CANDIDATE_TEMPLATES = [
    ...
    'Template.DOEP Enrollment Letter.docx',
    'Template.DOEP Enroll.docx',
    'Template.DOEP Entrance Notification.docx',
]

template_path = None
for name in CANDIDATE_TEMPLATES:
    template_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, name)
    if template_path:
        break

if not template_path:
    record_error(f"[FATAL] Missing enrollment template (any of {CANDIDATE_TEMPLATES}) under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
    sys.exit(0)  # soft-exit with no docs rather than error
