...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for AEMP clients.

Usage (CLI)
------------
python AEMP_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate AEMP Victim Notification Letters")
...
# env overrides (match BIPP pattern)
PROGRAM_CODE  = os.environ.get('NP_PROGRAM_CODE', 'AEMP')
...
LOG_PATH = os.path.join(LOG_DIR, f"AEMP_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("AEMP Victim Letters Script started.")
...
LETTER_TYPES = {
    "entrance": {
        "template": "Template.AEMP Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.AEMP Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.AEMP Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to AEMP programs
PROGRAM_FILTER = ["AEMP"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No AEMP records found – exiting.")
    sys.exit(0)
...
for idx, row in df.iterrows():
    ...
    if not template_file:
        ...
        # don’t crash the whole run; just skip this row
        continue
