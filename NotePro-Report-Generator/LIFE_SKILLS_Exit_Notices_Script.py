...
parser = argparse.ArgumentParser(description='Generate Life Skills Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'LIFE_SKILLS_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Life Skills Exit Notices Script started.")
...
exit_notice_template_path = os.path.join(templates_dir, 'Template.LIFE SKILLS Exit Notice.docx')
progress_report_template_path = os.path.join(templates_dir, 'Template.LIFE SKILLS Exit Progress Report.docx')
virtual_progress_report_template_path = os.path.join(templates_dir, 'Template.LIFE SKILLS Exit Progress Report Virtual.docx')
...
def generate_documents(row, output_dir_path):
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "Life Skills Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"Life Skills {report_suffix}", context, output_dir_path)
...
logging.info("Life Skills Exit Notices Script completed successfully.")
