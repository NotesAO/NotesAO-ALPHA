...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for DWII Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWII_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWII Behavior Contract Script started.")
...
def need_template(filename: str):
    ...
    return p  # may be None; caller must guard


# ------------------------------------------------------------------------------
# 4) Paths to Behavior Contract Template
# ------------------------------------------------------------------------------
behavior_contract_template_path = need_template('Template.DWII Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to DWII + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be DWII (male or female). Adjust if your CSV uses different strings.
dwii_filter = df['program_name'].isin(['DWII'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[dwii_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching DWII + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("DWII Behavior Contract Script completed successfully.")
