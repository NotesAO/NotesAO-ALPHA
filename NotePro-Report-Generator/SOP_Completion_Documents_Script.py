...
parser = argparse.ArgumentParser(description='Generate SOP Completion Documents')
...
completion_certificate_file_path = os.path.join(templates_dir, 'Template.SOP Completion Certificate.docx')
completion_letter_file_path = os.path.join(templates_dir, 'Template.SOP Completion Letter.docx')
completion_report_file_path = os.path.join(templates_dir, 'Template.SOP Completion Progress Report.docx')
completion_virtual_report_file_path = os.path.join(templates_dir, 'Template.SOP Completion Progress Report Virtual.docx')
...
if df_filtered.empty:
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            letter_docx = render_and_save(completion_letter_file_path, "SOP Comp Letter", context, generated_documents_dir)
            certificate_docx = render_and_save(completion_certificate_file_path, "SOP Comp Cert", context, generated_documents_dir)
            progress_template = completion_virtual_report_file_path if "Virtual" in row['group_name'] else completion_report_file_path
            progress_report_docx = render_and_save(progress_template, "SOP PReport", context, generated_documents_dir)
