...
parser = argparse.ArgumentParser(description='Generate DWIE Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWIE_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWIE Unexcused Absences Script started.")
...
# Template path
template_file_path = os.path.join(templates_dir, 'Template.DWIE Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} DWIE Unexcused Absence.docx"
...
logging.info("DWIE Unexcused Absences Script completed successfully.")
