"""
Life Skills_Victim_Letters_Script.py
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for Life Skills clients.

Usage (CLI)
------------
python Life Skills_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate Life Skills Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"LIFE_SKILLS_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("Life Skills Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.LIFE SKILLS Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.LIFE SKILLS Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.LIFE SKILLS Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to Life Skills programs
PROGRAM_FILTER = ["Life Skills"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No Life Skills records found – exiting.")
    sys.exit(0)
