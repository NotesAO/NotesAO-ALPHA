...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for WEST Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'WEST_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("WEST Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.WEST Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to WEST + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be WEST (male or female). Adjust if your CSV uses different strings.
west_filter = df['program_name'].isin(['WEST'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[west_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching WEST + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("WEST Behavior Contract Script completed successfully.")
