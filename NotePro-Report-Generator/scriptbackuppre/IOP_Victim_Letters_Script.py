...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for IOP clients.

Usage (CLI)
------------
python IOP_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate IOP Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"IOP_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("IOP Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.IOP Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.IOP Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.IOP Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to IOP programs
PROGRAM_FILTER = ["IOP"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No IOP records found – exiting.")
    sys.exit(0)
