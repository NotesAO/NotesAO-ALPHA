"""
MARI_EDU_Victim_Letters_Script.py
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for Marijuana Education clients.

Usage (CLI)
------------
python MARI_EDU_Victim_Letters_Script.py \
...
Required Word templates (names are fixed):
  ...
  Template.MARIJUANA EDU Victim Completion.docx
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate Marijuana Education Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"MARI_EDU_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("Marijuana Education Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.MARIJUANA EDU Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.MARIJUANA EDU Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.MARIJUANA EDU Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to Marijuana Education programs
PROGRAM_FILTER = ["Marijuana Education"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No Marijuana Education records found – exiting.")
    sys.exit(0)
