...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for OBSTRUCTION clients.

Usage (CLI)
------------
python OBSTRUCTION_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate OBSTRUCTION Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"OBSTRUCTION_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("OBSTRUCTION Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.OBSTRUCTION Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.OBSTRUCTION Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.OBSTRUCTION Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to OBSTRUCTION programs
PROGRAM_FILTER = ["OBSTRUCTION"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No OBSTRUCTION records found – exiting.")
    sys.exit(0)
