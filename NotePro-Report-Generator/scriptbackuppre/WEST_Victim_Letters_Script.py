...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for WEST clients.

Usage (CLI)
------------
python WEST_Victim_Letters_Script.py \
...
Required Word templates (names are fixed):
  Template.WEST Victim Entrance.docx
  Template.WEST Victim Completion.docx
  Template.WEST Victim Exit.docx
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate WEST Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"WEST_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("WEST Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.WEST Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.WEST Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.WEST Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to WEST programs
PROGRAM_FILTER = ["WEST"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No WEST records found – exiting.")
    sys.exit(0)
