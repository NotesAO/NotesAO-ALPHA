...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for MRT clients.

Usage (CLI)
------------
python MRT_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate MRT Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"MRT_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("MRT Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.MRT Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.MRT Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.MRT Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to MRT programs
PROGRAM_FILTER = ["MRT"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No MRT records found – exiting.")
    sys.exit(0)
