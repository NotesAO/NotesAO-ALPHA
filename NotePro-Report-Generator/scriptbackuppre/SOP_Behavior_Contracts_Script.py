...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for SOP Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'SOP_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("SOP Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.SOP Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to SOP + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be SOP (male or female). Adjust if your CSV uses different strings.
sop_filter = df['program_name'].isin(['SOP'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[sop_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching SOP + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("SOP Behavior Contract Script completed successfully.")
