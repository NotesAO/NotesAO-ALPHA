...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for OBSTRUCTION Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'OBSTRUCTION_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("OBSTRUCTION Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.OBSTRUCTION Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to OBSTRUCTION + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be OBSTRUCTION (male or female). Adjust if your CSV uses different strings.
obstruction_filter = df['program_name'].isin(['OBSTRUCTION'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[obstruction_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching OBSTRUCTION + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("OBSTRUCTION Behavior Contract Script completed successfully.")
