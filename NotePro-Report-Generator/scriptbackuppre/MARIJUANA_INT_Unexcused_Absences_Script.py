...
parser = argparse.ArgumentParser(description='Generate Marijuana Intervention Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MARI_INT_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Marijuana Intervention Unexcused Absences Script started.")
...
# Template path
template_file_path = os.path.join(templates_dir, 'Template.MARIJUANA INT Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} MARI INT Unexcused Absence.docx"
...
logging.info("Marijuana Intervention Unexcused Absences Script completed successfully.")
