"""
MARI_INT_Victim_Letters_Script.py
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for Marijuana Intervention clients.

Usage (CLI)
------------
python MARI_INT_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate Marijuana Intervention Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"MARI_INT_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("Marijuana Intervention Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.MARIJUANA INT Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.MARIJUANA INT Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.MARIJUANA INT Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to Marijuana Intervention programs
PROGRAM_FILTER = ["Marijuana Intervention"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No Marijuana Intervention records found – exiting.")
    sys.exit(0)
