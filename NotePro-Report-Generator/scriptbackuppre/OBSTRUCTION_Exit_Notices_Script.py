...
parser = argparse.ArgumentParser(description='Generate OBSTRUCTION Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'OBSTRUCTION_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("OBSTRUCTION Exit Notices Script started.")
...
exit_notice_template_path = os.path.join(templates_dir, 'Template.OBSTRUCTION Exit Notice.docx')
progress_report_template_path = os.path.join(templates_dir, 'Template.OBSTRUCTION Exit Progress Report.docx')
virtual_progress_report_template_path = os.path.join(templates_dir, 'Template.OBSTRUCTION Exit Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for OBSTRUCTION 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for OBSTRUCTION 27-week: attended={attended}")
            return 'Alert'
...
df = df[df['program_name'].isin(['OBSTRUCTION'])]
...
def generate_documents(row, output_dir_path):
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "OBSTRUCTION Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"OBSTRUCTION {report_suffix}", context, output_dir_path)
...
logging.info("OBSTRUCTION Exit Notices Script completed successfully.")
