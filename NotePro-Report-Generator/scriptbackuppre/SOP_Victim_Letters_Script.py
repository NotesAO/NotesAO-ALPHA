...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for SOP clients.

Usage (CLI)
------------
python SOP_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate SOP Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"SOP_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("SOP Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.SOP Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.SOP Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.SOP Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to SOP programs
PROGRAM_FILTER = ["SOP"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No SOP records found – exiting.")
    sys.exit(0)
