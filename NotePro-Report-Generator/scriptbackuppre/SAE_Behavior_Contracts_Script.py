...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for SAE Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'SAE_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("SAE Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.SAE Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to SAE + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be SAE (male or female). Adjust if your CSV uses different strings.
sae_filter = df['program_name'].isin(['SAE'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[sae_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching SAE + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("SAE Behavior Contract Script completed successfully.")
