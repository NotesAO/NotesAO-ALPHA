...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for Marijuana Intervention Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MARI_INT_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Marijuana Intervention Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.MARIJUANA INT Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to Marijuana Intervention + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be Marijuana Intervention (male or female). Adjust if your CSV uses different strings.
mari_int_filter = df['program_name'].isin(['Marijuana Intervention'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[mari_int_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching Marijuana Intervention + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("Marijuana Intervention Behavior Contract Script completed successfully.")
