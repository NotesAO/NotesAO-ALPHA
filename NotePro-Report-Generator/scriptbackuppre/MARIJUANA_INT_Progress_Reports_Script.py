...
parser = argparse.ArgumentParser(description='Generate Marijuana Intervention Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MARI_INT_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Marijuana Intervention Progress Reports Script started.")
...
progress_report_template_path = os.path.join(templates_dir, 'Template.MARIJUANA INT Progress Report.docx')
virtual_progress_report_template_path = os.path.join(templates_dir, 'Template.MARIJUANA INT Progress Report Virtual.docx')
...
# Filter for Marijuana Intervention (male/female) and Not Exited clients
...
if df.empty:
    logging.warning("No records found for Marijuana Intervention Not Exited clients. No documents will be generated.")
    sys.exit(0)
...
def render_and_save(template_path, doc_type, context, output_dir):
    try:
        ...
        docx_filename = f"{last_name} {first_name} MARIJUANA INT PReport.docx"
...
logging.info("Marijuana Intervention Progress Reports Script completed successfully.")
