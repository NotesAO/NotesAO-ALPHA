...
parser = argparse.ArgumentParser(description='Generate PARENTING Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'PARENTING_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("PARENTING Exit Notices Script started.")
...
exit_notice_template_path = os.path.join(templates_dir, 'Template.PARENTING Exit Notice.docx')
progress_report_template_path = os.path.join(templates_dir, 'Template.PARENTING Exit Progress Report.docx')
virtual_progress_report_template_path = os.path.join(templates_dir, 'Template.PARENTING Exit Progress Report Virtual.docx')
...
def determine_stage_of_change(attended_str, required_str):
    ...
    if required == 18:
        if 1 <= attended <= 4:
            ...
            logging.warning(f"Unexpected attendance value for PARENTING 18-week: attended={attended}")
            return 'Alert'
    ...
    if required == 27:
        if 1 <= attended <= 7:
            ...
            logging.warning(f"Unexpected attendance value for PARENTING 27-week: attended={attended}")
            return 'Alert'
...
df = df[df['program_name'].isin(['Parenting Education', 'Parenting'])]
...
def generate_documents(row, output_dir_path):
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "PARENTING Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"PARENTING {report_suffix}", context, output_dir_path)
...
logging.info("PARENTING Exit Notices Script completed successfully.")
