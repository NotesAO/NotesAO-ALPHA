...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for AC clients.

Usage (CLI)
------------
python AC_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate AC Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"AC_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("AC Victim Letters Script started.")
...
LETTER_TYPES = {
    "entrance": {
        "template": "Template.AC Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.AC Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.AC Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to AC programs
PROGRAM_FILTER = ["Anger Control", "Anger (male)", "Anger (female)", "Anger Management"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No AC records found – exiting.")
    sys.exit(0)
...
for idx, row in df.iterrows():
    ...
    if not template_file:
        ...
        continue  # skip this client/victim row, keep generating others
