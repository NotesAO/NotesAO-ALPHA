...
parser = argparse.ArgumentParser(description='Generate SAE Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'SAE_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("SAE Unexcused Absences Script started.")
...
# Template path
template_file_path = os.path.join(templates_dir, 'Template.SAE Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} SAE Unexcused Absence.docx"
...
logging.info("SAE Unexcused Absences Script completed successfully.")
