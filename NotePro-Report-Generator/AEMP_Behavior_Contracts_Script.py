...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for AEMP Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'AEMP_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("AEMP Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = need_template('Template.AEMP Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to AEMP + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be AEMP (male or female). Adjust if your CSV uses different strings.
AEMP_filter = df['program_name'].isin(['AEMP'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[AEMP_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching AEMP + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
for idx, row in df.iterrows():
    try:
        ...
        record_error(f"Row {idx} failed: {e}")  # continue to next
# no sys.exit here; non-fatal per-row errors


logging.info("AEMP Behavior Contract Script completed successfully.")
