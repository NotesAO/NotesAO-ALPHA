...
parser = argparse.ArgumentParser(description='Generate AEMP Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'AEMP_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("AEMP Unexcused Absences Script started.")
...
# Template path
template_file_path = need_template('Template.AEMP Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} AEMP Unexcused Absence.docx"
...
logging.info("AEMP Unexcused Absences Script completed successfully.")
