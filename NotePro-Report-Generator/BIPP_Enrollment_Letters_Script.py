...
# --- Env overrides to match other scripts ---
...
CANDIDATE_TEMPLATES = [
    ...
    'Template.BIPP Enrollment Letter.docx',
    'Template.BIPP Enroll.docx',
    'Template.BIPP Entrance Notification.docx',  # last-resort fallback
]
...
# when trying candidates:
template_path = None
for candidate in CANDIDATE_TEMPLATES:
    p = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, candidate)
    if p:
        template_path = p
        break

if not template_path:
    record_error(f"[FATAL] Missing enrollment template(s) under clinic/default fallbacks in {_TEMPLATES_ROOT}. Tried: {CANDIDATE_TEMPLATES}")
    sys.exit(0)
...
# ---- Column normalization / presence checks (soft-fail) ----
required_anyway = ['enrollment_date', 'first_name', 'last_name']
cm_expected = ['case_manager_office', 'case_manager_first_name', 'case_manager_last_name']
...
cm_missing = [c for c in cm_expected if c not in df.columns]

if missing:
    record_error(f"CSV missing required Enrollment columns: {missing} → exiting clean.")
    sys.exit(0)

if cm_missing:
    # Not fatal—letters can still render, but foldering may default to Unknown
    record_error(f"CSV missing case manager columns (will default to Unknown where needed): {cm_missing}")


# Normalize DOB column to a single 'dob'
if 'dob' not in df.columns and 'date_of_birth' in df.columns:
    df['dob'] = df['date_of_birth']
df['dob'] = df['dob'].astype(str).str.strip() if 'dob' in df.columns else ''
...
# Filter by date range (inclusive of entire end day)
end_cutoff = end_date + timedelta(days=1)
df_filtered = df[(df['enrollment_date_dt'] >= start_date) & (df['enrollment_date_dt'] < end_cutoff)]
logging.info(f"Filtered to date range [{start_date:%Y-%m-%d} .. {end_date:%Y-%m-%d} incl.]. Rows: {len(df_filtered)}")
...
for idx, row in df_filtered.iterrows():
    try:
        ...
        record_error(f"[row {idx}] Render failed for {row.get('last_name','')}, {row.get('first_name','')}: {e}")
...
for docx_path, row in generated_docx_files:
    try:
        ...
        if pdf_path:
            ...
            cm_office  = sanitize_filename(raw_office)
            ...
            cm_first   = sanitize_filename(raw_first)
            cm_last    = sanitize_filename(raw_last)
            cm_full    = (f"{cm_first} {cm_last}".strip() or (cm_last if cm_last else 'Unknown_Manager'))
...
print("Enrollment letters complete.")
sys.exit(0)
