...
parser = argparse.ArgumentParser(description='Generate DOEP Completion Documents')
...
program_code  = os.environ.get('NP_PROGRAM_CODE', 'DOEP')
...
completion_certificate_file_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Completion Certificate.docx')
if not completion_certificate_file_path:
    record_error(f"[FATAL] Missing template Template.DOEP Completion Certificate.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")

completion_letter_file_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Completion Letter.docx')
if not completion_letter_file_path:
    record_error(f"[FATAL] Missing template Template.DOEP Completion Letter.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")

completion_report_file_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Completion Progress Report.docx')
if not completion_report_file_path:
    record_error(f"[FATAL] Missing template Template.DOEP Completion Progress Report.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")

completion_virtual_report_file_path = resolve_template(_TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Completion Progress Report Virtual.docx')
if not completion_virtual_report_file_path:
    record_error(f"[FATAL] Missing template Template.DOEP Completion Progress Report Virtual.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
...
df = df[df['program_name'].isin(['DOEP'])]
...
if df_filtered.empty:
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            letter_docx = render_and_save(completion_letter_file_path, "DOEP Comp Letter", context, generated_documents_dir)
            certificate_docx = render_and_save(completion_certificate_file_path, "DOEP Comp Cert", context, generated_documents_dir)
            progress_template = completion_virtual_report_file_path if "Virtual" in row['group_name'] else completion_report_file_path
            progress_report_docx = render_and_save(progress_template, "DOEP PReport", context, generated_documents_dir)
