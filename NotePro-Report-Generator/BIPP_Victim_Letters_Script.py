...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for BIPP clients.

Usage (CLI)
------------
python BIPP_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate BIPP Victim Notification Letters")
...
CLINIC        = os.environ.get('NP_CLINIC_FOLDER', CLINIC)

# after env overrides (PROGRAM_CODE/TEMPLATES_DIR/CLINIC) add:
if os.path.basename(TEMPLATES_DIR) == CLINIC:
...
ERRORS_TXT = os.path.join(CENTRAL_DIR, 'errors.txt')
...
LOG_PATH = os.path.join(LOG_DIR, f"BIPP_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("BIPP Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.BIPP Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.BIPP Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.BIPP Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
def resolve_template(templates_root: str, clinic: str, program: str, filename: str) -> Optional[str]:
    """
    Prefer clinic template; fall back to default/{program}; then default/BIPP.
...
# Restrict to BIPP programs
PROGRAM_FILTER = ["BIPP (male)", "BIPP (female)"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No BIPP records found – exiting.")
    sys.exit(0)
...
for idx, row in df.iterrows():
    try:
        ...
        # Gender placeholders
        gender_key = (row_dict.get("gender") or "").strip().lower()
        gender_map = PRONOUNS.get(gender_key, {
            "gender1": "their","gender2": "they","gender3": "Participants",
            "gender4": "them","gender5": "They","gender6": "Their","gender7": ""
        })
        ...
        # Template path with clinic → default/{PROGRAM_CODE} → default/BIPP fallback
        tmpl_name = LETTER_TYPES[applicable_type]["template"]
        template_file = resolve_template(_TEMPLATES_ROOT, CLINIC, PROGRAM_CODE, tmpl_name)
        if not template_file:
            record_error(f"[row {idx}] Missing template {tmpl_name} under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
            continue
        ...
        for v_idx in victim_slots:
            ...
            try:
                ...
                record_error(f"[row {idx}] Render failed (victim slot {v_idx}): {e}")
                continue
            ...
            if pdf_path:
                ...
                record_error(f"Conversion to PDF failed: {doc_path}")

                continue

            # Organize as: /Victim Letters/<Office>/<Manager>/<Client>
            ...
            victim_parent = "Victim Letters"

            raw_office = pick(row_dict, 'case_manager_office','facilitator_office','instructor_office', default='')
            office_dir = os.path.join(CENTRAL_DIR, victim_parent, sanitize_filename(raw_office or 'Unknown Office'))

            raw_first = pick(row_dict, 'case_manager_first_name','facilitator_first_name','instructor_first_name', default='')
            raw_last  = pick(row_dict, 'case_manager_last_name', 'facilitator_last_name', 'instructor_last_name',  default='')
            manager_sub = (f"{sanitize_filename(raw_first)} {sanitize_filename(raw_last)}".strip()
            ...
            client_folder = (f"{client_first} {client_last}".strip() or 'Unknown Client')

            target_dir = os.path.join(office_dir, manager_sub, client_folder)
        ...
        record_error(f"[row {idx}] Unexpected error: {e}")
        continue
