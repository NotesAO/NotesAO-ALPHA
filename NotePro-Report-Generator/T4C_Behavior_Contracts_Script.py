...
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Generate Behavior Contracts for T4C Clients')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'T4C_Behavior_Contract_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("T4C Behavior Contract Script started.")
...
# ------------------------------------------------------------------------------
behavior_contract_template_path = os.path.join(templates_dir, 'Template.T4C Behavior Contract.docx')
...
# ------------------------------------------------------------------------------
# 7) Filter to T4C + Behavior Contract Status
# ------------------------------------------------------------------------------
# Must be T4C (male or female). Adjust if your CSV uses different strings.
t4c_filter = df['program_name'].isin(['Thinking for a Change'])

# behavior_contract_status must be "Needed" or "Signed"
bc_filter = df['behavior_contract_status'].isin(['Needed', 'Signed'])

df = df[t4c_filter & bc_filter]

if df.empty:
    logging.warning("No records found matching T4C + Behavior Contract 'Needed' or 'Signed'. Exiting.")
    sys.exit(0)
...
logging.info("T4C Behavior Contract Script completed successfully.")
