...
parser = argparse.ArgumentParser(description='Generate AEMP Exit Notices')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'AEMP_Exit_Notices_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("AEMP Exit Notices Script started.")
...
exit_notice_template_path            = need_template('Template.AEMP Exit Notice.docx')
progress_report_template_path        = need_template('Template.AEMP Exit Progress Report.docx')
virtual_progress_report_template_path= need_template('Template.AEMP Exit Progress Report Virtual.docx')
...
def generate_documents(row, output_dir_path):
    ...
    exit_pdf = render_and_save(exit_notice_template_path, "AEMP Exit Notice", context, output_dir_path)
    # Generate Exit Progress Report
    progress_pdf = render_and_save(report_template, f"AEMP {report_suffix}", context, output_dir_path)
...
logging.info("AEMP Exit Notices Script completed successfully.")
