...
parser = argparse.ArgumentParser(description='Generate DWI Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DWI_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DWI Progress Reports Script started.")
...
progress_report_template_path         = need_template('Template.DWI Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.DWI Progress Report Virtual.docx')
...
# Filter for DWI (male/female) and Not Exited clients
...
if df.empty:
    logging.warning("No records found for DWI Not Exited clients. No documents will be generated.")
    sys.exit(0)
...
def render_and_save(template_path, doc_type, context, output_dir):
    try:
        ...
        docx_filename = f"{last_name} {first_name} DWI Stage PReport.docx"
...
def generate_documents(row, output_dir_path):
    ...
    if not report_template:
        return  # skip this row; error logged already
...
logging.info("DWI Progress Reports Script completed successfully.")
