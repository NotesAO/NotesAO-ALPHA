...
parser = argparse.ArgumentParser(description='Generate IOP Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'IOP_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("IOP Unexcused Absences Script started.")
...
# Template path
template_file_path = need_template('Template.IOP Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    if not template_file_path:
        record_error("Cannot render IOP Unexcused Absence – template missing.")
        return
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} IOP Unexcused Absence.docx"
...
logging.info("IOP Unexcused Absences Script completed successfully.")
