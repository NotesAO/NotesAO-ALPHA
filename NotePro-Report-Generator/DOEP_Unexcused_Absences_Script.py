...
parser = argparse.ArgumentParser(description='Generate DOEP Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'DOEP_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("DOEP Unexcused Absences Script started.")

base_dir = os.path.dirname(os.path.abspath(__file__))
templates_dir = args.templates_dir

# --- env overrides + templates root (match other programs) ---
...
# Template path
template_file_path = resolve_template(
    _TEMPLATES_ROOT, clinic_folder, program_code, 'Template.DOEP Unexcused Absence.docx'
)
if not template_file_path:
    record_error(f"[FATAL] Missing template Template.DOEP Unexcused Absence.docx under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} DOEP Unexcused Absence.docx"
...
logging.info("DOEP Unexcused Absences Script completed successfully.")
