...
parser = argparse.ArgumentParser(description='Generate DWIE Completion Documents')
...
completion_certificate_file_path        = need_template('Template.DWIE Completion Certificate.docx')
completion_letter_file_path             = need_template('Template.DWIE Completion Letter.docx')
completion_report_file_path             = need_template('Template.DWIE Completion Progress Report.docx')
completion_virtual_report_file_path     = need_template('Template.DWIE Completion Progress Report Virtual.docx')
...
def render_and_save(template_path, doc_type, context, output_dir):
    if not template_path:
        record_error(f"[FATAL] Cannot render {doc_type} – missing template.")
        return None
    try:
...
if df_filtered.empty:
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            letter_docx = render_and_save(completion_letter_file_path, "DWIE Comp Letter", context, generated_documents_dir)
            certificate_docx = render_and_save(completion_certificate_file_path, "DWIE Comp Cert", context, generated_documents_dir)
            progress_template = completion_virtual_report_file_path if "Virtual" in row['group_name'] else completion_report_file_path
            progress_report_docx = render_and_save(progress_template, "DWIE PReport", context, generated_documents_dir)
