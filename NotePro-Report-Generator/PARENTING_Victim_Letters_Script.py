...
--------------------------------
Generate Entrance, Completion, or Exit victim notification letters (DOCX ➜ PDF) for PARENTING clients.

Usage (CLI)
------------
python PARENTING_Victim_Letters_Script.py \
...
# ------------------------- CLI PARSING -------------------------
parser = argparse.ArgumentParser(description="Generate PARENTING Victim Notification Letters")
...
LOG_PATH = os.path.join(LOG_DIR, f"PARENTING_Victim_Letters_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(filename=LOG_PATH, level=logging.DEBUG, format="%(asctime)s [%(levelname)s] %(message)s")
logging.info("PARENTING Victim Letters Script started.")

# ------------------------- CONSTANTS -------------------------
LETTER_TYPES = {
    "entrance": {
        "template": "Template.PARENTING Victim Entrance.docx",
    ...
    "completion": {
        "template": "Template.PARENTING Victim Completion.docx",
        "criteria": lambda row, sd, ed: (
    ...
    "exit": {
        "template": "Template.PARENTING Victim Exit.docx",
        "criteria": lambda row, sd, ed: (
...
# Restrict to PARENTING programs
PROGRAM_FILTER = ["Parenting Education", "Parenting"]
df = df[df["program_name"].isin(PROGRAM_FILTER)]
if df.empty:
    logging.warning("No PARENTING records found – exiting.")
    sys.exit(0)
