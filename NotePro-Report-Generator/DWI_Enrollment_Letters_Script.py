...
templates_dir = os.environ.get('NP_NP_TEMPLATES_DIR', templates_dir)  # if typo, fix to NP_TEMPLATES_DIR
...
CANDIDATE_TEMPLATES = [
    ...
    'Template.DWI Enrollment Letter.docx',
    'Template.DWI Enroll.docx',
    'Template.DWI Entrance Notification.docx',  # last-resort fallback
]

template_path = None
for name in CANDIDATE_TEMPLATES:
    p = need_template(name)
    if p:
        template_path = p
        break

if not template_path:
    record_error(f"[FATAL] No enrollment DOCX template found (tried {CANDIDATE_TEMPLATES}) under clinic/default fallbacks in {_TEMPLATES_ROOT}.")
    sys.exit(0)  # graceful no-op
