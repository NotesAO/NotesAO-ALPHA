...
parser = argparse.ArgumentParser(description='Generate ADV PARENTING Progress Reports')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'ADVANCE_Parenting_Progress_Reports_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("ADV PARENTING Progress Reports Script started.")
...
progress_report_template_path         = need_template('Template.ADV PARENTING Progress Report.docx')
virtual_progress_report_template_path = need_template('Template.ADV PARENTING Progress Report Virtual.docx')
...
# Filter for ADV PARENTING (male/female) and Not Exited clients
...
if df.empty:
    logging.warning("No records found for ADV PARENTING Not Exited clients. No documents will be generated.")
    sys.exit(0)
...
def render_and_save(template_path, doc_type, context, output_dir):
    try:
        ...
        docx_filename = f"{last_name} {first_name} ADV PARENTING Stage PReport.docx"
...
logging.info("ADV PARENTING Progress Reports Script completed successfully.")
