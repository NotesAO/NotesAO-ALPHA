...
parser = argparse.ArgumentParser(description='Generate Marijuana Education Unexcused Absences Documents')
...
logging.basicConfig(
    filename=os.path.join(log_dir, f'MARI_EDU_Unexcused_Absences_{datetime.now().strftime("%Y%m%d")}.log'),
    level=logging.DEBUG,
    format='%(asctime)s [%(levelname)s] %(message)s'
)
logging.info("Marijuana Education Unexcused Absences Script started.")
...
# Template path
template_file_path = os.path.join(templates_dir, 'Template.MARIJUANA EDU Unexcused Absence.docx')
...
def generate_documents(row, output_dir_path):
    ...
    try:
        ...
        docx_filename = f"{last_name} {first_name} MARIJUANA EDU Unexcused Absence.docx"
...
logging.info("Marijuana Education Unexcused Absences Script completed successfully.")
