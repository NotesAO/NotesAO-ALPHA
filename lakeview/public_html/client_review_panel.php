<?php
    ...
    // fetch program flags (override milestone logic by program_id)
    $prog = sql_select_one($con, "SELECT p.id AS program_id
                                        FROM client c JOIN program p ON p.id=c.program_id
                                    WHERE c.id=?", [$client_id]);

    $program_id = (int)($prog['program_id'] ?? 0);

    // Only these programs use milestones: DOEP(1), Parenting(4), Life Skills/Anti Theft(6)
    $MILESTONE_PROGRAM_IDS = [1, 4, 6];
    $uses_milestones = in_array($program_id, $MILESTONE_PROGRAM_IDS) ? 1 : 0;
    ...
    if (!empty($client['program_id'])) {
      $program_name = fetch_label_by_id($con, 'program', 'id', $client['program_id'], ['name','program','title']);
        ...
        // Grouping plan per program
        $MILESTONE_GROUPING = [
            4 => [4,4],   // Parenting Education
            6 => [4,4],   // Life Skills / Anti-Theft
            1 => 5,       // DOEP example: groups of 5 (adjust if needed)
            // 3 => [5,5,5], // Example: DWII hybrid
        ];
        ...
        // Expected days of week per program when weekly attends_* flags are not set
        // 0=Sun ... 6=Sat
        $EXPECTED_DOWS = [
        ...
        if ($uses_milestones) {
            ...
            $curriculum = sql_select_all(
                $con, "SELECT * FROM curriculum WHERE program_id=? ORDER BY id ASC", [$program_id]
            );
            // completed ids
            $progress_rows = sql_select_all(
                $con, "SELECT curriculum_id AS cid FROM client_milestone WHERE client_id=?", [$client_id]
            );
            ...
            $done_rows = sql_select_all(
                $con, "SELECT curriculum_id AS cid, DATE_FORMAT(completed_at,'%Y-%m-%d %H:%i') AS completed_at
                      FROM client_milestone WHERE client_id=?", [$client_id]
            );
            ...
            // date bound for back/forward dating checkboxes
        ...
        // Last attended + next expected for Missed box
  ...
  <div class="row">
    ...
    <div class="row bg-light">
      <div class="col-12 bg-white">
        <!-- TOP ROW: left = cards, right = milestones OR attendance calendars -->
        <div class="row">
          ...
          <div class="col-8 pt-3 pb-3">
            ...
            <div class="card mb-3">
              ...
              <div class="card-body">
                <?php
                  // Keep the same order and green-for-expected logic
          ...
          <!-- RIGHT: milestones (if enabled) OR attendance calendars -->
          <div class="col-4 pt-3 pb-3">
            ...
            <!-- Missed box (works for both modes) + Photo under it -->
            <div class="mt-4">
              <h6>Missed</h6>
              ...
              <!-- Photo placed directly below the note -->
              <div class="mt-3">
