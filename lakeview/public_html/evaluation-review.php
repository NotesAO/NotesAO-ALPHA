  ...
  foreach ($keys as $k) {
    ...
    if ($raw === '' || $raw === null) {
      ...
      if (stripos($title, 'sassi') !== false && ($s === '0' || $s === '1')) {
        $val = ($s === '1') ? 'True' : 'False'; // 1=True, 0=False
      } else {
        $val = h((string)$raw);
      }
...
function vtc_label_for(string $k): string {
  // Override only especially long/cryptic keys; fall back to vtc_labelize for everything else.
  static $map = [
    // HAM-D examples (add more as you like)
    'depressed_mood_gloomy_attitude_pessimism_about_the_future_feelin' => 'Depressed Mood / Pessimism',
    'feelings_of_guilt'                                                => 'Feelings of Guilt',
    'suicide'                                                          => 'Suicide',
    'insomnia_initial_difficulty_in_falling_asleep'                    => 'Insomnia — Initial',
    'insomnia_middle_complains_of_being_restless_and_disturbed_during' => 'Insomnia — Middle',
    'insomnia_delayed_waking_in_early_hours_of_the_morning_and_unable' => 'Insomnia — Late/Early Waking',
    'work_and_interests'                                               => 'Work & Interests',
    'retardation_slowness_of_thought_speech_and_activity_apathy_stupo' => 'Psychomotor Retardation',
    'agitation_restlessness_associated_with_anxiety'                   => 'Agitation',
    'anxiety'                                                          => 'Anxiety (Psychic)',
    'anxiety_somatic_gastrointestinal_indigestion_cardiovascular_palp' => 'Anxiety (Somatic)',
    'somatic_symptoms_gastrointestinal_loss_of_appetite_heavy_feeling' => 'Somatic Symptoms — GI',
    'somatic_symptoms_general'                                         => 'Somatic Symptoms — General',
    'genital_symptoms_loss_of_libido_menstrual_disturbances'           => 'Genital Symptoms / Libido',
    'hypochondriasis'                                                  => 'Hypochondriasis',
    'weight_loss'                                                      => 'Weight Loss',
    'insight_insight_must_be_interpreted_in_terms_of_patients_underst' => 'Insight',

    // You can add similar overrides for HAM-A / PCL-M / TBI if desired.
  ];

  $k2 = (string)$k;
  if (isset($map[$k2])) return $map[$k2];
  return vtc_labelize($k2);
}
...
/* =========================
 * Lakeview — generic render blocks
 * ========================= */

/* Generic card wrapper around render_kv_table() */
function lv_render_generic(string $title, ?array $row, array $exclude = []): string {
  if (!$row) return '<div class="text-muted">No data.</div>';
  $html  = '<div class="card mb-3"><div class="card-header fw-bold">'.h($title).'</div><div class="card-body">';
  $html .= render_kv_table($row, $exclude);
  $html .= '</div></div>';
  return $html;
}

/* SASSI renderer: groups by prefixes if present in table */
function lv_render_sassi(?array $row): string {
  if (!$row) return '<div class="text-muted">No SASSI submission for this client.</div>';

  // Try to group by known prefixes from your SASSI form
  $groups = [
    'SASSI — True/False Statements' => function($k){ return preg_match('/^(tf_|sassi_tf_)/i',$k); },
    'SASSI — Alcohol Behaviors (FVA)' => function($k){ return preg_match('/^(fva_|sassi_fva_)/i',$k); },
    'SASSI — Drug/Medication Behaviors (FVD)' => function($k){ return preg_match('/^(fvd_|sassi_fvd_)/i',$k); },
  ];

  $out = '';
  foreach ($groups as $label => $pred) {
    $keys = [];
    foreach ($row as $k => $v) if ($pred((string)$k)) $keys[] = $k;
    sort($keys, SORT_NATURAL|SORT_FLAG_CASE);
    if (!$keys) continue;

    $out .= '<div class="mb-3"><h6 class="mb-2">'.h($label).'</h6>';
    $out .= '<div class="table-responsive"><table class="table table-sm table-bordered mb-0" style="table-layout:fixed;width:100%"><tbody>';
    foreach ($keys as $i => $k) {
      $val = $row[$k];
      $val = ($val==='' || $val===null) ? '<span class="text-muted">—</span>' : nl2br(h((string)$val));
      $out .= '<tr><th style="width:64px" class="text-center">'.($i+1).'</th><td>'.h(vtc_labelize($k)).'</td><td style="width:180px" class="text-center">'.$val.'</td></tr>';
    }
    $out .= '</tbody></table></div></div>';
  }

  // Show any non-grouped leftovers (excluding demographics/metadata)
  $exclude = array_merge($GLOBALS['exclude_keys_common'], array_keys($row));
  // ^ quick way to suppress extra table if everything matched; we’ll rebuild a lighter exclude:
  $common = $GLOBALS['exclude_keys_common'];
  $left = [];
  foreach ($row as $k=>$v) {
    if (in_array($k,$common,true)) continue;
    if (preg_match('/^(tf_|sassi_tf_|fva_|sassi_fva_|fvd_|sassi_fvd_)/i',(string)$k)) continue;
    $left[$k]=$v;
  }
  if ($left) {
    $out .= '<div class="mb-3"><h6 class="mb-2">Other Responses</h6>';
    $out .= render_kv_table($left, []);
    $out .= '</div>';
  }

  return $out ?: '<div class="text-muted">No SASSI fields detected.</div>';
}
...
/* Lakeview evaluations (tables exist per your uploaded scripts) */
$BDI_LV_CFG = [
  'table'    => 'evaluations_bdi',
  'pk'       => 'id',
  'first'    => ['first_name','name_first','fname'],
  'last'     => ['last_name','name_last','lname'],
  'dob'      => ['dob','date_of_birth','birth_date'],
  'email'    => ['email','email_address'],
  'created_candidates' => ['submitted_at','created_at','created','ts'],
  'updated'  => ['updated_at'],
];
...
$VALID_FORMS = ['combined','bdi','dast','sassi','dwii_a','dwii_b','dwii_exit','ndp'];

if (!in_array($form,$VALID_FORMS,true)) $form = 'combined';

$key  = $_GET['key'] ?? '';
$pai_id_sel = isset($_GET['pai_id']) ? trim((string)$_GET['pai_id']) : null;
$vtc_id_sel = isset($_GET['vtc_id']) ? trim((string)$_GET['vtc_id']) : null;

// We only key on FIRST + LAST now (no DOB).
$who = ['first'=>'','last'=>''];
if ($key !== '') {
  $raw = base64_decode(strtr($key, ' ', '+'));
...
$bdi_lv     = resolve_columns($link, $BDI_LV_CFG);
$dast       = resolve_columns($link, $DAST_CFG);
$dwii_a     = resolve_columns($link, $DWII_A_CFG);
$dwii_b     = resolve_columns($link, $DWII_B_CFG);
$dwii_exit  = resolve_columns($link, $DWII_EXIT_CFG);
$ndp        = resolve_columns($link, $NDP_CFG);
$sassi      = resolve_columns($link, $SASSI_CFG);

if (!$bdi_lv)    { /* optional: $errors[] = "BDI table not found."; */ }
if (!$dast)      { /* optional */ }
if (!$dwii_a)    { /* optional */ }
if (!$dwii_b)    { /* optional */ }
if (!$dwii_exit) { /* optional */ }
if (!$ndp)       { /* optional */ }
if (!$sassi)     { /* optional */ }

$pai_missing = !$pai;
$vtc_missing = !$vtc;
// remove the two lines that pushed these into $errors
// if (!$pai) $errors[] = "...";
// if (!$vtc) $errors[] = "...";
...
// final errors
$errors = [];
if ($first_key==='' || $last_key==='') $errors[] = 'Missing or invalid client key (first, last).';
 ...
 * ------------------------------------------------------------------*/
function fetch_form_rows(mysqli $db, array $meta, string $first_key, string $last_key): array {
  if (!$meta || !$meta['id'] || !$meta['first'] || !$meta['last']) return [];
  $t=$meta['table']; $id=$meta['id']; $fn=$meta['first']; $ln=$meta['last']; $cr=$meta['created'] ?: $id;

  $sql = "SELECT * FROM `$t`
          WHERE LOWER(TRIM(`$fn`))=? AND LOWER(TRIM(`$ln`))=?
          ORDER BY `$cr` DESC";
  $stmt = $db->prepare($sql);
  if (!$stmt) return [];
...
/**
 * Returns date_of_birth from intake_packet only for evals that carry intake_id.
 * Otherwise returns null so the UI can display "—".
 */
function eval_dob(mysqli $db, string $table, int $eval_id): ?string {
    if ($table === 'evaluations_bdi' || $table === 'evaluations_dast') {
        $sql = "SELECT ip.date_of_birth
                  FROM `$table` e
                  JOIN intake_packet ip ON ip.intake_id = e.intake_id
                 WHERE e.id = ?";
        $stmt = $db->prepare($sql);
    ...
    // DWII_A, DWII_B, DWII_EXIT, NDP, SASSI: no DOB in schema
    return null;
}

/** SASSI exposes age, others do not. */
function eval_age(mysqli $db, string $table, int $eval_id): ?int {
    if ($table === 'evaluations_sassi') {
        $stmt = $db->prepare("SELECT `age` FROM evaluations_sassi WHERE id = ?");
        $stmt->bind_param('i', $eval_id);
        $stmt->execute();
        $row = $stmt->get_result()->fetch_assoc();
        return isset($row['age']) ? (int)$row['age'] : null;
    }
    return null;
}
...
$pai_rows = $pai ? fetch_form_rows($link, $pai, $first_key, $last_key) : [];
$vtc_rows = $vtc ? fetch_form_rows($link, $vtc, $first_key, $last_key) : [];

$bdi_lv_rows    = $bdi_lv    ? fetch_form_rows($link, $bdi_lv,    $first_key, $last_key) : [];
$dast_rows      = $dast      ? fetch_form_rows($link, $dast,      $first_key, $last_key) : [];
$dwii_a_rows    = $dwii_a    ? fetch_form_rows($link, $dwii_a,    $first_key, $last_key) : [];
$dwii_b_rows    = $dwii_b    ? fetch_form_rows($link, $dwii_b,    $first_key, $last_key) : [];
$dwii_exit_rows = $dwii_exit ? fetch_form_rows($link, $dwii_exit, $first_key, $last_key) : [];
$ndp_rows       = $ndp       ? fetch_form_rows($link, $ndp,       $first_key, $last_key) : [];
$sassi_rows     = $sassi     ? fetch_form_rows($link, $sassi,     $first_key, $last_key) : [];
...
$one_bdi_lv    = pick_row($bdi_lv_rows,    isset($_GET['bdi_id'])     ? (string)$_GET['bdi_id']     : null);
$one_dast      = pick_row($dast_rows,      isset($_GET['dast_id'])    ? (string)$_GET['dast_id']    : null);
$one_dwii_a    = pick_row($dwii_a_rows,    isset($_GET['dwii_a_id'])  ? (string)$_GET['dwii_a_id']  : null);
$one_dwii_b    = pick_row($dwii_b_rows,    isset($_GET['dwii_b_id'])  ? (string)$_GET['dwii_b_id']  : null);
$one_dwii_exit = pick_row($dwii_exit_rows, isset($_GET['dwii_exit_id']) ? (string)$_GET['dwii_exit_id'] : null);
$one_ndp       = pick_row($ndp_rows,       isset($_GET['ndp_id'])     ? (string)$_GET['ndp_id']     : null);
$one_sassi     = pick_row($sassi_rows,     isset($_GET['sassi_id'])   ? (string)$_GET['sassi_id']   : null);
...
// Ensure core demo fields use key values when empty (DOB is not keyed)
...
<title>Evaluation Review — <?=h($demographics['last_name']?($demographics['last_name'].', '.$demographics['first_name']):'Client')?></title>
...
<main class="container-fluid pt-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-2">
      ...
      <div>
        <div class="btn-group" role="group" aria-label="view">
          ...
          <a class="btn btn-sm btn-outline-primary<?= $form==='bdi'?' active':'' ?>" href="<?=$base.'&form=bdi'?>">BDI</a>
          <a class="btn btn-sm btn-outline-primary<?= $form==='dast'?' active':'' ?>" href="<?=$base.'&form=dast'?>">DAST</a>
          <a class="btn btn-sm btn-outline-primary<?= $form==='sassi'?' active':'' ?>" href="<?=$base.'&form=sassi'?>">SASSI</a>
          <a class="btn btn-sm btn-outline-primary<?= $form==='dwii_a'?' active':'' ?>" href="<?=$base.'&form=dwii_a'?>">DWII-A</a>
          <a class="btn btn-sm btn-outline-primary<?= $form==='dwii_b'?' active':'' ?>" href="<?=$base.'&form=dwii_b'?>">DWII-B</a>
          <a class="btn btn-sm btn-outline-primary<?= $form==='dwii_exit'?' active':'' ?>" href="<?=$base.'&form=dwii_exit'?>">DWII Exit</a>
          <a class="btn btn-sm btn-outline-primary<?= $form==='ndp'?' active':'' ?>" href="<?=$base.'&form=ndp'?>">NDP</a>
        </div>
  ...
  <div class="card mb-3">
    <div class="card-body">
      <div>
        <?php
        $key_status = ($errors ? 'Client key not resolved' : 'Client key matched by First + Last');

        ?>
        <h4 class="mb-1"><?=h($demographics['last_name'])?>, <?=h($demographics['first_name'])?></h4>

        <div class="text-muted small"><?=h($key_status)?></div>
      </div>
        ...
        <div class="mt-2 mt-md-0">
          ...
          <span class="chip <?= $one_bdi_lv?'ok':'miss' ?> mr-1"><i class="fas <?= $one_bdi_lv?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>BDI <?= $one_bdi_lv ? ('• '.h(fmt_dt($one_bdi_lv[$bdi_lv['created']] ?? ''))) : 'Missing' ?></span>
          <span class="chip <?= $one_dast?'ok':'miss' ?> mr-1"><i class="fas <?= $one_dast?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>DAST <?= $one_dast ? ('• '.h(fmt_dt($one_dast[$dast['created']] ?? ''))) : 'Missing' ?></span>
          <span class="chip <?= $one_sassi?'ok':'miss' ?> mr-1"><i class="fas <?= $one_sassi?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>SASSI <?= $one_sassi ? ('• '.h(fmt_dt($one_sassi[$sassi['created']] ?? ''))) : 'Missing' ?></span>
          <span class="chip <?= $one_dwii_a?'ok':'miss' ?> mr-1"><i class="fas <?= $one_dwii_a?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>DWII-A <?= $one_dwii_a ? ('• '.h(fmt_dt($one_dwii_a[$dwii_a['created']] ?? ''))) : 'Missing' ?></span>
          <span class="chip <?= $one_dwii_b?'ok':'miss' ?> mr-1"><i class="fas <?= $one_dwii_b?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>DWII-B <?= $one_dwii_b ? ('• '.h(fmt_dt($one_dwii_b[$dwii_b['created']] ?? ''))) : 'Missing' ?></span>
          <span class="chip <?= $one_dwii_exit?'ok':'miss' ?> mr-1"><i class="fas <?= $one_dwii_exit?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>DWII Exit <?= $one_dwii_exit ? ('• '.h(fmt_dt($one_dwii_exit[$dwii_exit['created']] ?? ''))) : 'Missing' ?></span>
          <span class="chip <?= $one_ndp?'ok':'miss' ?>"><i class="fas <?= $one_ndp?'fa-check-circle text-success':'fa-times-circle text-danger'?>"></i>NDP <?= $one_ndp ? ('• '.h(fmt_dt($one_ndp[$ndp['created']] ?? ''))) : 'Missing' ?></span>
...
<div class="col-6 col-lg-1">
  ...
  <?php if ($form !== 'combined'): ?>
    <?php
      // Map form key -> metadata sets
      $FORM_MAP = [
        'pai'       => ['meta'=>$pai,      'rows'=>$pai_rows,      'one'=>$one_pai,      'title'=>'PAI Evaluation',     'id'=>'pai_id'],
        'vtc'       => ['meta'=>$vtc,      'rows'=>$vtc_rows,      'one'=>$one_vtc,      'title'=>'VTC Evaluation',     'id'=>'vtc_id'],
        'bdi'       => ['meta'=>$bdi_lv,   'rows'=>$bdi_lv_rows,   'one'=>$one_bdi_lv,   'title'=>'BDI (Lakeview)',     'id'=>'bdi_id'],
        'dast'      => ['meta'=>$dast,     'rows'=>$dast_rows,     'one'=>$one_dast,     'title'=>'DAST',               'id'=>'dast_id'],
        'sassi'     => ['meta'=>$sassi,    'rows'=>$sassi_rows,    'one'=>$one_sassi,    'title'=>'SASSI',              'id'=>'sassi_id'],
        'dwii_a'    => ['meta'=>$dwii_a,   'rows'=>$dwii_a_rows,   'one'=>$one_dwii_a,   'title'=>'DWII — Part A',      'id'=>'dwii_a_id'],
        'dwii_b'    => ['meta'=>$dwii_b,   'rows'=>$dwii_b_rows,   'one'=>$one_dwii_b,   'title'=>'DWII — Part B',      'id'=>'dwii_b_id'],
        'dwii_exit' => ['meta'=>$dwii_exit,'rows'=>$dwii_exit_rows,'one'=>$one_dwii_exit,'title'=>'DWII — Exit',        'id'=>'dwii_exit_id'],
        'ndp'       => ['meta'=>$ndp,      'rows'=>$ndp_rows,      'one'=>$one_ndp,      'title'=>'NDP',                'id'=>'ndp_id'],
      ];
      $cfg = $FORM_MAP[$form] ?? null;
      $meta = $cfg['meta'] ?? null; $rows = $cfg['rows'] ?? []; $one = $cfg['one'] ?? null; $title = $cfg['title'] ?? strtoupper($form); $idParam = $cfg['id'] ?? 'id';
      $idFld = $meta['id'] ?? 'id';
      $crFld = $meta['created'] ?? $idFld;
    ?>
    <div class="card mb-3">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div><strong><?= h($title) ?></strong> <?= $one? '<span class="text-muted small">(Submitted '.h(fmt_dt($one[$crFld] ?? '')).')</span>' : '' ?></div>
        <div>
          <?php if (count($rows)>1): ?>
            <form method="get" class="form-inline my-0">
              ...
              <select name="<?= h($idParam) ?>" class="form-control form-control-sm" onchange="this.form.submit()">
                <?php foreach($rows as $r): $optId=$r[$idFld] ?? reset($r); $label = ($r[$crFld] ?? '') ? fmt_dt($r[$crFld]) : ('ID '.h((string)$optId)); ?>
      ...
      <div class="card-body">
        ...
        <?php if ($one): ?>
          ...
          <?php elseif ($form==='sassi'): ?>
            <?= lv_render_sassi($one) ?>
          <?php else: /* All other Lakeview forms as generic KV */ ?>
            <?= lv_render_generic($title, $one, $exclude_keys_common) ?>
          <?php endif; ?>
        <?php else: ?>
  ...
  <?php else: ?>
    ...
    <div class="row">
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>BDI (Lakeview)</strong> <?= $one_bdi_lv? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_bdi_lv[$bdi_lv['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($bdi_lv_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="bdi">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="bdi_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($bdi_lv_rows as $r): $optId=$r[$bdi_lv['id']] ?? reset($r); $label = ($r[$bdi_lv['created']] ?? '') ? fmt_dt($r[$bdi_lv['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_bdi_lv && (string)($one_bdi_lv[$bdi_lv['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_bdi_lv ? render_kv_table($one_bdi_lv, $exclude_keys_common) : '<div class="text-muted">No BDI submission for this client.</div>' ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>DAST</strong> <?= $one_dast? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_dast[$dast['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($dast_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="dast">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="dast_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($dast_rows as $r): $optId=$r[$dast['id']] ?? reset($r); $label = ($r[$dast['created']] ?? '') ? fmt_dt($r[$dast['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_dast && (string)($one_dast[$dast['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_dast ? render_kv_table($one_dast, $exclude_keys_common) : '<div class="text-muted">No DAST submission for this client.</div>' ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>SASSI</strong> <?= $one_sassi? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_sassi[$sassi['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($sassi_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="sassi">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="sassi_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($sassi_rows as $r): $optId=$r[$sassi['id']] ?? reset($r); $label = ($r[$sassi['created']] ?? '') ? fmt_dt($r[$sassi['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_sassi && (string)($one_sassi[$sassi['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_sassi ? lv_render_sassi($one_sassi) : '<div class="text-muted">No SASSI submission for this client.</div>' ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>DWII — Part A</strong> <?= $one_dwii_a? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_dwii_a[$dwii_a['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($dwii_a_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="dwii_a">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="dwii_a_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($dwii_a_rows as $r): $optId=$r[$dwii_a['id']] ?? reset($r); $label = ($r[$dwii_a['created']] ?? '') ? fmt_dt($r[$dwii_a['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_dwii_a && (string)($one_dwii_a[$dwii_a['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_dwii_a ? render_kv_table($one_dwii_a, $exclude_keys_common) : '<div class="text-muted">No DWII-A submission for this client.</div>' ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>DWII — Part B</strong> <?= $one_dwii_b? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_dwii_b[$dwii_b['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($dwii_b_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="dwii_b">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="dwii_b_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($dwii_b_rows as $r): $optId=$r[$dwii_b['id']] ?? reset($r); $label = ($r[$dwii_b['created']] ?? '') ? fmt_dt($r[$dwii_b['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_dwii_b && (string)($one_dwii_b[$dwii_b['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_dwii_b ? render_kv_table($one_dwii_b, $exclude_keys_common) : '<div class="text-muted">No DWII-B submission for this client.</div>' ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>DWII — Exit</strong> <?= $one_dwii_exit? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_dwii_exit[$dwii_exit['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($dwii_exit_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="dwii_exit">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="dwii_exit_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($dwii_exit_rows as $r): $optId=$r[$dwii_exit['id']] ?? reset($r); $label = ($r[$dwii_exit['created']] ?? '') ? fmt_dt($r[$dwii_exit['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_dwii_exit && (string)($one_dwii_exit[$dwii_exit['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_dwii_exit ? render_kv_table($one_dwii_exit, $exclude_keys_common) : '<div class="text-muted">No DWII Exit submission for this client.</div>' ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="card mb-3">
          <div class="card-header d-flex align-items-center justify-content-between">
            <div><strong>NDP</strong> <?= $one_ndp? '<span class="text-muted small">(Submitted '.h(fmt_dt($one_ndp[$ndp['created']] ?? '')).')</span>' : '<span class="text-danger small">Missing</span>' ?></div>
            <div>
              <?php if (count($ndp_rows)>1): ?>
                <form method="get" class="form-inline my-0">
                  <input type="hidden" name="key" value="<?=h($key)?>">
                  <input type="hidden" name="form" value="ndp">
                  <label class="mr-2 mb-0 small">Submission:</label>
                  <select name="ndp_id" class="form-control form-control-sm" onchange="this.form.submit()">
                    <?php foreach($ndp_rows as $r): $optId=$r[$ndp['id']] ?? reset($r); $label = ($r[$ndp['created']] ?? '') ? fmt_dt($r[$ndp['created']]) : ('ID '.h((string)$optId)); ?>
                      <option value="<?=h((string)$optId)?>" <?= ($one_ndp && (string)($one_ndp[$ndp['id']]??'')===(string)$optId)?'selected':'' ?>><?=h($label)?></option>
                    <?php endforeach; ?>
                  </select>
                </form>
              <?php endif; ?>
          ...
          <div class="card-body">
            <?= $one_ndp ? render_kv_table($one_ndp, $exclude_keys_common) : '<div class="text-muted">No NDP submission for this client.</div>' ?>
          </div>
        </div>
      </div>
    </div>
