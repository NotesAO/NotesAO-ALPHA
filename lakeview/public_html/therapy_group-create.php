...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("INSERT INTO therapy_group (program_id,name,address,city,state,zip) VALUES (?,?,?,?,?,?)");

    if ($stmt->execute([$_SESSION['program_id'], $name, $address, $city, $state, $zip])) {
        $stmt = null;
        header("location: therapy_group-index.php");
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>NotesAO - Therapy Group Create</title>
