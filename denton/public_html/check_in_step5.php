...
//                header("location: attendance_record-index.php");
        ...
        $stmt = $pdo->prepare("UPDATE client set email = ?, phone_number = ?, fee = ?, case_manager_id = ?, client_stage_id = ?, other_concerns = ? WHERE ID = ?");
        if($stmt->execute([ $email, $phone_number, $fee, $case_manager_id, $client_stage_id, $other_concerns, $client_id ])) {
            $stmt = null;
//                header("location: attendance_record-index.php");
        } else{
            echo "Something went wrong. Please try again later.";
