...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE therapy_session SET therapy_group_id=?,date=?,duration_minutes=?,curriculum_id=?,facilitator_id=?,note=? WHERE id=?");

    if(!$stmt->execute([ $therapy_group_id,$date,$duration_minutes,$curriculum_id,$facilitator_id,$note,$id  ])) {
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
						...
						<div class="form-group">
                                <label>curriculum</label>
                                    ...
                                    <?php
                                        $sql = "SELECT short_description, id FROM curriculum";
                                        $result = mysqli_query($link, $sql);
                                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            $duprow = $row;
                                            unset($duprow["id"]);
